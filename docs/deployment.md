...
| **Container image** (`FASTPKI_IMAGE`) | Where this server gets the FastPKI container image. **On a single server, press Enter** to keep `fastpki:local`: the installer builds the image on this server from the source you downloaded. This needs nothing else, but it is slow, because it compiles four C/C++ projects. A name that contains `/`, for example `registry.example.org/fastpki:1.0`, is **downloaded** from that registry instead of built. Run `docker login` first if the registry is private. Use a registry when you build the image once and install many servers from it (§3.3). |
...
**1. Name the CA.** Both values are your issuing CA (`issuing-ca` below). In the console this is
the **Config** tab; on the command line:

```bash
cd ~/FastPKI/deploy
docker compose exec web fastpki-config set PG_TLS_CA_ID issuing-ca
docker compose exec web fastpki-config set CMP_CLIENT_CA_ID issuing-ca
```
...
The install is two steps, and they are separate on purpose:
...
# or, directly:
IMAGE=ghcr.io/fastpki/fastpki:v0.1.0-rc14 PKI_DNS=pki.example.org bash deploy/k8s/apply.sh
```
...
```
IMAGE=ghcr.io/fastpki/fastpki:v0.1.0-rc14
```
...
```bash
cd ~/FastPKI && git checkout v0.1.0-rc14
```
...
|---|---|---|
| `IMAGE`, `IMAGE_PULL_POLICY` | `fastpki:latest`, `IfNotPresent` | which image, and whether to re-pull. The default names a locally built image; a published release is `ghcr.io/fastpki/fastpki:VERSION` (§8.0 step 3) |
