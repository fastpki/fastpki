...
int main(int argc, char** argv) {
    ...
    try {
        ...
        auto pick_responder = [&](const std::string& id, const std::string& host,
            ...
            // ⚠️ THE RESPONDER CERTIFICATE BELONGS IN THE REFERENCE TOO. A Responder loads
            // OCSP_RESPONDER_KEY once, in its constructor. Replacing that key (the HA
            // procedure deletes it and renew-service-certs --create-missing makes a replicable
            // one) issues a new responder certificate for the new key, and a Responder built
            // before it went on holding the old key: every request was refused with "does not
            // match the OCSP responder key this process holds" until a restart. A new
            // certificate serial now rebuilds it, and the new Responder loads the new key.
            std::string responder_ref;
            if (!cfg.ocsp_responder_key.empty()) {
                try {
                    auto der = db->get_cert_by_cert_id(cfg.ocsp_responder_cert_id_prefix + "-" + rc.id);
                    if (der && !der->empty()) {
                        const unsigned char* p = der->data();
                        pki::X509Ptr x{d2i_X509(nullptr, &p, static_cast<long>(der->size()))};
                        if (x) responder_ref = pki::x509_serial_hex(x.get());
                    }
            ...
            if (rit != inst_resp_ref.end() && rit->second != mat_ref) {
                pki::log::info("OCSP instance '" + rc.id + "': CA material or responder "
                               "certificate changed — rebuilding the responder");
        ...
        // GET /ocsp/<base64 of the DER request>  — RFC 6960 §A.1, RFC 5019.
        //
        // ⚠️ ROUTED FROM THE RAW TARGET, NOT req.path. The base64 is URL-encoded by a compliant
        // client, so a '/' in it arrives as %2F — and httplib DECODES the path before routing.
        // /ocsp/AAA%2FBBB became /ocsp/AAA/BBB: it matched the per-CA form below with the
        // first chunk of the base64 as a CA id, and answered 404. The issuer-hash half of a
        // CertID is the same for every request to one CA, so for a CA whose hashes encode to
        // a '/', EVERY GET failed — measured on two lab CAs, 110 requests out of 110 — while
        // POST worked. Windows CryptoAPI asks over GET.
        //
        // So one route takes everything after /ocsp/ from req.target, still escaped. When
        // the first raw segment names a CA, it is the per-CA form /ocsp/{id}/{base64}; any
        // other shape is the base64 of the shared responder — which also serves a client
        // that sends the base64 unescaped, '/' and all.
        auto url_unescape = [](const std::string& in) -> std::string {
            // ⚠️ NO std::stoi HERE. It throws std::invalid_argument on a non-hex pair, and a
            // `%zz` then escaped as an exception and the client got 500 (a server fault)
            ...
            for (size_t i = 0; i < in.size(); ++i) {
                int hi = -1, lo = -1;
                if (in[i] == '%' && i + 2 < in.size() &&
                    (hi = nib(in[i + 1])) >= 0 && (lo = nib(in[i + 2])) >= 0) {
        ...
        // The DER request from the escaped base64; "" = malformed.
        auto get_req_der = [&](const std::string& escaped) -> std::string {
            const std::string b64 = url_unescape(escaped);
            BIO* b64bio = BIO_new(BIO_f_base64());
            BIO_set_flags(b64bio, BIO_FLAGS_BASE64_NO_NL);
            BIO* mem = BIO_new_mem_buf(b64.data(), static_cast<int>(b64.size()));
            b64bio = BIO_push(b64bio, mem);
            std::vector<unsigned char> der(b64.size());
        ...
        auto do_get = [&](pki::ocsp::Responder& r, const std::string& der, httplib::Response& res) {
        ...
        srv.Get(R"(/ocsp/.+)", [&](const httplib::Request& req, httplib::Response& res) {
            std::string raw = req.target;
            if (const auto q = raw.find('?'); q != std::string::npos) raw.resize(q);
            static const std::string kPrefix = "/ocsp/";
            if (raw.compare(0, kPrefix.size(), kPrefix) != 0) { res.status = 404; return; }
            const std::string rest = raw.substr(kPrefix.size());
            const std::string host = req.get_header_value("Host");
            // /ocsp/{id}/{base64} — a per-CA responder. Same reasoning as the CRL: a relying
            // party asking about a certificate signed by a now-disabled CA needs an answer.
            if (const auto slash = rest.find('/'); slash != std::string::npos) {
                const std::string id = url_unescape(rest.substr(0, slash));
                if (pki::resolve_ca_instance(*db, cfg, id).found) {
                    const std::string der = get_req_der(rest.substr(slash + 1));
                    if (der.empty()) { res.status = 400; return; }
                    if (auto r = pick_responder(id, host, res, nullptr, /*require_active=*/false))
                        do_get(*r, der, res);
                    return;
                }
            }
            // /ocsp/{base64} — the shared responder, chosen from the request as for POST.
            const std::string der = get_req_der(rest);
            if (der.empty()) { res.status = 400; return; }
            if (auto r = responder_for_request(host, der, res)) do_get(*r, der, res);
        });
