...
# See docs/manual-procedures.md §6 and the patch files themselves for the full reasoning.
#
...
# The checksums of the three patched libraries, so install-native.sh can refuse a host where
# Alpine's stock build has replaced one (an `apk add p11-kit` after the package was unpacked
# did exactly that, and the token then offered no EdDSA or ML-DSA keys).
for _lib in /usr/lib/libp11-kit.so.0 /usr/lib/pkcs11/p11-kit-client.so /usr/lib/softhsm/libsofthsm2.so; do
    printf '%s  %s\n' "$(sha256sum "${DESTDIR}$_lib" | cut -d' ' -f1)" "$_lib"
done > "$WORK/patched-libs.sha256"
inst 0644 "$WORK/patched-libs.sha256" /usr/share/fastpki/patched-libs.sha256
...
# The no-downtime restore, run on the standby. It detects a native host on its own.
inst 0755 "$ROOT/deploy/db-restore-online.sh" /usr/share/fastpki/db-restore-online.sh
