...
# Before any grant: a token with NO profile resolves to the identity's default at
# enrolment, and an identity with no profile has none — so minting one must fail here too.
# It used to be checked only when a profile was named, and the token then failed on the
# device (a Mac reported only "Unable to obtain certificate from SCEP server").
# The identity's profiles come from its own console role AND from line 94's grant, so both
# are taken away for this check and put back straight after it.
ungrant_profile scep requester
pg_exec "UPDATE web_users SET role='none' WHERE username='scep';" >/dev/null
NONE=$(pg_exec "SELECT count(*) FROM role_permissions p
                 WHERE p.permission='profile:use' AND (p.role IN (SELECT role FROM web_users WHERE username='scep')
                    OR p.role IN (SELECT role FROM subject_roles WHERE selector_value='scep'));")
chk "PRECONDITION: the scep identity holds no profile" 0 "$NONE"
NOPROF=$("$ROOT/build/fastpki-scep" --config dyn.conf --issue-challenge --ttl 3600 2>/dev/null)
chk "  a token with no profile is REFUSED at mint while the identity has none" "" "$NOPROF"
pg_exec "UPDATE web_users SET role='requester' WHERE username='scep';" >/dev/null
NOPROF=$("$ROOT/build/fastpki-scep" --config dyn.conf --issue-challenge --ttl 3600 2>/dev/null)
chk "  ... and minted once the identity has a profile again" yes "$([ -n "$NOPROF" ] && echo yes || echo no)"
grant_profile scep requester
