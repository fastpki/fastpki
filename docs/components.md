...
revokes, and rolls over its account key. A Mac, iPhone or iPad enrols through the same
service with device attestation instead (see *Device attestation* below).
...
- **Device attestation (`device-attest-01`)** — for Apple devices, from an ACME configuration
 profile. An administrator issues a one-time ticket (`fastpki-acme --issue-device-ticket`); the
 device sends it as a `permanent-identifier` (or, for an MDM fleet, sends its serial number, registered
 with `/api/device-serials`), and answers the challenge with an Apple-signed
 attestation that is verified in the request: it must chain to the Apple Enterprise
 Attestation Root (compiled in) or a root in `ACME_ATTESTATION_ROOTS`, and carry SHA-256 of the
 challenge token. The certificate is issued for the attested key only, to the ticket's owner,
 and the device's previous certificate is revoked as superseded. An account without an External
 Account Binding is accepted only while a ticket is outstanding, and it can place device
 orders only. [`protocol-apis.md`](protocol-apis.md) has the full flow.
...
SCEP (RFC 8894) enrols network devices (Cisco and others), MDM-managed devices, and Macs,
iPhones and iPads from a configuration profile. It runs over
...
console uses the values in the `config` table. `verify` checks a detached signature, and it is
the only step that checks one: `check` reads the feed and compares version numbers, and nothing
verifies an artifact on your behalf.

Each release's `SHA256SUMS` is signed twice — `SHA256SUMS.sig` with ECDSA P-256, readable by
any OpenSSL, and `SHA256SUMS.mldsa.sig` with post-quantum ML-DSA-87, which needs OpenSSL 3.5 or
newer. Either verifying is enough. `SHA256SUMS` carries the hash of every artifact, so once its
signature checks, `sha256sum -c` proves each file. Both public keys are compiled into the programs, so `verify` works with no
configuration; `RELEASE_PUBKEY` overrides them for a deployment signing its own builds. The
keys are also in `docs/release-keys/` for checking a download with `openssl` directly.
`SHA256SUMS` catches a corrupted download but not a substituted one, because it travels the
same channel as the artifact. Applying an update
