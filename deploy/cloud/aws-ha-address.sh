...
#
#   ./aws-ha-address.sh create --deployment fastpki --node 1 --ssh alpine@…
...
Options:
  ...
  --ssh USER@HOST      create, move: how to reach the node that should hold the address,
                       to configure it there — without this the address exists in AWS and
...
# ⚠️ THE BOOT SCRIPT ASKS AWS WHETHER THIS NODE STILL HOLDS THE ADDRESS, and it must: a
# failover away from a host that is DOWN cannot reach that host to take the address off it.
# The host comes back, runs a boot script that adds the address unconditionally, and now two
# machines hold the pair's address at operating-system level while the VPC routes it to one
# of them. The returning standby answers on it locally — a read-only database serving what
# should have reached the primary — and on a path where an address is claimed on the wire
# instead of by the VPC, the same shape is a duplicate-address conflict.
#
# Elsewhere an arbiter settles this: keepalived brings a returning node up as a backup, so
# it does not claim the VIP. A VPC has no such thing, and this script is what stands in for
# it. The instance metadata service is the authority, it needs no credentials, and it is
# reachable from the node itself, so every node can settle its own case at every boot. It
# removes a stale address as readily as it adds a live one, which is what lets a returning
# host converge without anyone visiting it.
#
# If the metadata service cannot be read at all, it adds the address. Of the two ways to be
# wrong, a primary that does not configure the address is an outage, while a standby that
# does is the residue described above — and an unreachable IMDS on a running EC2 instance is
# close to impossible.
boot_script() {
  cat <<EOF
#!/bin/sh
# Written by deploy/cloud/aws-ha-address.sh. The pair's shared address belongs to whichever
# instance AWS routes it to. Ask at every boot and follow the answer, so a host that returns
# after a failover gives the address up by itself.
ADDR=$1
IMDS=http://169.254.169.254/latest
mac=\$(cat /sys/class/net/eth0/address 2>/dev/null)
tok=\$(curl -sf -X PUT "\$IMDS/api/token" \\
        -H 'X-aws-ec2-metadata-token-ttl-seconds: 60' --max-time 5 2>/dev/null)
if [ -n "\$mac" ] && mine=\$(curl -sf -H "X-aws-ec2-metadata-token: \$tok" \\
        "\$IMDS/meta-data/network/interfaces/macs/\$mac/ipv6s" --max-time 5 2>/dev/null) \\
   && ! printf '%s\\n' "\$mine" | grep -qx "\$ADDR"; then
    ip -6 addr del "\$ADDR/128" dev eth0 2>/dev/null
    exit 0
fi
ip -6 addr add "\$ADDR/128" dev eth0 2>/dev/null
exit 0
EOF
}

# Install that script on a host and run it once, so the address is configured now and again
# at every boot. `create` and `move` both need exactly this, on the node the address has just
# been assigned to.
configure_on_host() {
  local host="$1" addr="$2"
  echo "Configuring ${addr} on ${host}…" >&2
  if [ "$DRY" = 1 ]; then
    echo "+ ssh ${host} 'write /etc/local.d/fastpki-ha-address.start, then run it'" >&2
    return 0
  fi
  boot_script "$addr" | "${SSH[@]}" "$host" \
    "R=${ROOT}; \$R tee /etc/local.d/fastpki-ha-address.start >/dev/null; \
     \$R chmod +x /etc/local.d/fastpki-ha-address.start; \
     \$R rc-update add local default >/dev/null 2>&1 || true; \
     \$R /etc/local.d/fastpki-ha-address.start"
}
...
create)
  ...
  if [ -n "$SSH_NEW" ]; then
    configure_on_host "$SSH_NEW" "$addr" \
      || die "could not configure the address on ${SSH_NEW} — it is assigned in AWS but nothing answers on it yet"
    step1="1. Done: ${SSH_NEW} holds ${addr} and will reclaim it at every boot."
  else
    step1="1. Configure it on the node that holds it. The address exists in AWS, but the
     operating system knows nothing of it until it is on the interface, and nothing
     answers on it until then. Run this again with --ssh <user@that node>."
  fi
  cat >&2 <<EOF
...
move)
  ...
  if [ -n "$SSH_NEW" ]; then
    ...
    echo "so nothing answers on it, and it will not come back after a reboot. Run this again" >&2
    echo "with --ssh <user@the new holder>, or on that host:" >&2
