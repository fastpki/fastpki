...
| `csr <id>` | Build a CSR for a CA whose key is in **this** node's token, to be signed by a root held elsewhere — the multi-data-center bootstrap (deployment.md §9.1) |
...
| `verify <artifact> <sig> [--pubkey <pem>]` | Verify a release's detached signature against the PEM public key in `--pubkey`, else `RELEASE_PUBKEY` |
