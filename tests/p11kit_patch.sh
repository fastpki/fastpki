...
chk "docs/manual-procedures.md names the patch file" yes \
    "$(grep -q 'deploy/p11-kit-mechanisms.patch' docs/manual-procedures.md && echo yes || echo no)"
chk "docs/manual-procedures.md shows how to apply it" yes \
    "$(grep -q 'patch -p1 --forward -d /tmp/p11kit -i' docs/manual-procedures.md && echo yes || echo no)"
chk "the guidance is in the native-install section" yes \
    "$(awk '/^## [0-9]+\. Native install/{s=1; next} /^## [0-9]+\. /{s=0} s' docs/manual-procedures.md \
        | grep -q 'p11-kit-mechanisms.patch' && echo yes || echo no)"
chk "it says which algorithms are affected" yes \
    "$(grep -q 'CKM_EC_EDWARDS_KEY_PAIR_GEN' docs/manual-procedures.md && echo yes || echo no)"
...
# Sections 1-5 all read TEXT — the patch file, the Dockerfile, docs/manual-procedures.md, and an
