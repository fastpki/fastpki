...
#   DEPLOYMENT=single|cluster   PKI_DNS=  PG_BIND=
#   DC_INDEX=<i>                        (cluster only; i in 1..32767, unique across the mesh)
#   KEY_BACKEND=softhsm|hsm             PKCS11_MODULE=<abs path>  PKCS11_TOKEN=  (when hsm)
...
if [ "$PRINT_ONLY" = 0 ]; then
  ...
  # The patched p11-kit and SoftHSM libraries must be the ones on disk. Alpine's stock builds
  # load and run, but drop the EdDSA and ML-DSA mechanisms, so Ed25519, Ed448 and ML-DSA CA
  # keys would be reported as absent from the token with nothing else wrong.
  if [ -r /usr/share/fastpki/patched-libs.sha256 ]; then
    ( cd / && sha256sum -c /usr/share/fastpki/patched-libs.sha256 >/dev/null 2>&1 ) \
      || die "Alpine's stock p11-kit or SoftHSM has replaced the patched build this package installed ($(cd / && sha256sum -c /usr/share/fastpki/patched-libs.sha256 2>/dev/null | grep -v ': OK$' | cut -d: -f1 | tr '\n' ' ')) — unpack the FastPKI package again (tar xzf fastpki-native-<version>-<arch>.tar.gz -C /), then run this again"
  fi
fi
...
if [ "$DEPLOYMENT" = cluster ]; then
  ...
  [ "$DC_INDEX" -ge 1 ] || die "DC_INDEX must be >= 1"
  [ "$DC_INDEX" -le 32767 ] || die "DC_INDEX must be <= 32767 (serial prefix is 2 octets, high bit reserved)"
...
# Asked here, straight after the data center, as install.sh asks it.
...
# ⚠️ AN HA PAIR'S DATABASE HOST LIST MUST SURVIVE A RE-RUN, for exactly the reason the PIN
# and the password must. deploy/ha-join.sh rewrites PG_CONNINFO AFTER the install to name
# BOTH hosts with target_session_attrs=read-write, and that line IS the failover: libpq
# tries each host and uses whichever one currently accepts writes, so a promotion needs no
# reconfiguration anywhere. Rebuilding the line from PG_HOST alone points the node back at
# its own Postgres. On whichever node is the standby that database is read-only, so every
# service there loses the ability to write, and fastpki-web cannot start AT ALL — it prunes
# expired sessions as it starts, that is a DELETE, and supervise-daemon then restarts it
# forever. Measured on a live pair: the documented update (docs/admin-guide.md §14.4)
# silently undid the join, and the console on the standby had been looping for 20 hours.
_PREV_PG_HOSTS=""; _PREV_PG_PORTS=""; _PREV_PG_TSA=""
if [ -r "$CONF" ]; then
  _PREV_PG_HOSTS="$(sed -n 's/^PG_CONNINFO=.*host=\([^ ]*\).*/\1/p' "$CONF" | head -1 || true)"
  _PREV_PG_PORTS="$(sed -n 's/^PG_CONNINFO=.* port=\([^ ]*\).*/\1/p'    "$CONF" | head -1 || true)"
  _PREV_PG_TSA="$(sed -n 's/^PG_CONNINFO=.* target_session_attrs=\([^ ]*\).*/\1/p' "$CONF" | head -1 || true)"
fi
if [ -n "$_PREV_PIN" ]; then
...
case "$KEY_BACKEND" in softhsm|hsm) ;; *) die "KEY_BACKEND must be 'softhsm' or 'hsm'" ;; esac
...
# The module of your own HSM, asked last, as install.sh asks it.
...
# A host list with a comma in it means ha-join.sh has been here and this node is half of a
# pair, so its list, ports and target_session_attrs are kept. Only when PG_LOCAL=yes, where
# PG_HOST is this installer's own 127.0.0.1 and never something the operator typed — with
# PG_LOCAL=no the address just answered is the one they meant, and it wins.
_PG_TSA=""
case "$_PREV_PG_HOSTS" in
  *,*)
    if [ "$PG_LOCAL" = yes ]; then
      PG_HOST="$_PREV_PG_HOSTS"
      PG_PORT="${_PREV_PG_PORTS:-$PG_PORT}"
      _PG_TSA=" target_session_attrs=${_PREV_PG_TSA:-read-write}"
      echo "  HA pair: keeping the database host list from the previous install ($PG_HOST)" >&2
    fi ;;
esac
PG_CONNINFO="host=$PG_HOST port=$PG_PORT dbname=fastpki user=fastpki password=$PG_PASSWORD sslmode=verify-full sslrootcert=$PG_SSLROOTCERT$_PG_TSA connect_timeout=5"
...
if [ "$PG_LOCAL" = yes ]; then
  ...
  if [ ! -f "$PGDATA/PG_VERSION" ]; then
    # ⚠️ EVERY DIRECTORY ABOVE THE DATA DIRECTORY MUST BELONG TO postgres FIRST. Alpine's
    # init script makes the whole path with one `install -d -o postgres … <parent>/17/data`,
    # and `install -d` applies the owner to the LAST component only — so `17` comes out
    # root:root 0700 and initdb fails with `could not access directory …: Permission denied`.
    # On an ordinary host the package created those directories long before; they appear
    # root-owned whenever /var/lib/postgresql is a freshly mounted volume, which is every
    # cloud server. The cloud first boot used to fix this itself, but on Alpine's own image
    # PostgreSQL is not installed yet when the volume is mounted, so there is no postgres
    # user and no version to create the directory for. This is the first point at which both
    # exist, on every path.
    # mkdir, then chown and chmod explicitly: `install -d` on a directory that already
    # exists — the mount point itself — is not guaranteed to change its owner.
    _pgparent="$(dirname "$PGDATA")"
    mkdir -p "$_pgparent"
    chown postgres:postgres "$(dirname "$_pgparent")" "$_pgparent"
    chmod 0750 "$(dirname "$_pgparent")" "$_pgparent"
...
# ── 8. services ───────────────────────────────────────────────────────────────────────
# ⚠️ crond, OR NOTHING RENEWS. The package installs /etc/periodic/daily/fastpki-certrenew,
# which renews the TLS certificates FastPKI issues to its own services and copies HA keys
# between the two servers of a pair — and busybox crond, which runs /etc/periodic/daily, is off
# on a fresh Alpine. Only the cloud image build turned it on. A native install on plain
# Alpine, the path §7 documents and a cloud server on Alpine's own image both take, therefore
# installed the daily job and never ran it: the certificates expired on schedule with nothing
# in any log, and a standby never received a key created after it joined.
#
# Enabled and started here, so it holds for every native install whatever it started from.
# Idempotent on the image, where it is already on.
rc-update add crond default >/dev/null 2>&1 || true
if [ "$DO_START" = 1 ]; then
  rc-service crond status >/dev/null 2>&1 || rc-service crond start >/dev/null 2>&1 \
    || echo "  WARN: crond did not start — the daily certificate renewal will not run until it does" >&2
fi
...
# format and both mesh passes. All of it is in docs/deployment.md §7 and §9 and docs/manual-procedures.md §6 and §8, and it
...
#
# And a node whose database holds a CA is not a first install either, meshed or not: a
# single node upgraded in place was still told to sign in as admin / admin and that no CA
# existed. _cas counts the rows `fastpki-ca list` shows, as apply.sh does. No answer counts as
# zero, which prints the first-install text.
_meshed=no
if [ "$(psql -h "$PG_HOST" -p "$PG_PORT" -U fastpki -d fastpki -tAqc \
          'SELECT count(*) FROM datacenters' 2>/dev/null)" -gt 1 ] 2>/dev/null; then
  _meshed=yes
fi
_cas=$(psql -h "$PG_HOST" -p "$PG_PORT" -U fastpki -d fastpki -tAqc \
         'SELECT count(DISTINCT id) FROM certs WHERE is_ca AND id IS NOT NULL' 2>/dev/null \
       | tr -d '[:space:]' || true)
...
This node: data center $DC_INDEX — $_keyline, $_sansline.
It is already part of a mesh and keeps replicating; there is nothing to connect.
EOF
elif [ "$DEPLOYMENT" = cluster ]; then
  cat <<EOF

This node: data center $DC_INDEX — that index IS its certificate-serial prefix
  — $_keyline, $_sansline.
...
This node: datacenter 1 (single), $_keyline. $_cas CA instance(s) present.
EOF
else
  cat <<EOF
