...
Download the one matching the node — an arm64 node needs the arm64 package — check it against
the release's `SHA256SUMS`, and copy it to the node:
...
```bash
curl -fsSLO https://github.com/fastpki/fastpki/releases/download/<version>/fastpki-native-<version>-<arch>.tar.gz
curl -fsSLO https://github.com/fastpki/fastpki/releases/download/<version>/SHA256SUMS
...
A native install needs no package: it builds on the node itself.
...
1. Install the new programs and PKCS#11 components over the old ones:
   ...
   - **A node started from the cloud image**: `tar xzf fastpki-native-<version>-<arch>.tar.gz -C /`.
