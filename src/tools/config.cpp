...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (cmd == "set") {
            ...
            // ⚠️ A KEY THAT NAMES A CA IS CHECKED AGAINST THE CAs THAT EXIST. Every one of
            // these is consumed as an id, and a value naming no CA does not fail loudly
            // anywhere — it makes the thing the key exists for quietly never happen. The
            // documented install sequence creates the CAs (§4.3) and then names them here
            // (§4.4), and the guide's example id differs from the id the same guide tells a
            // multi-data-center operator to use, so typing the example verbatim wrote a
            // dangling PG_TLS_CA_ID, the database certificate was never issued, and nothing
            // said so. Refusing costs nothing: the CA is always created first.
            //
            // Listed rather than merely rejected, because the operator's next question is
            // "then what IS it called?" and the answer is one query away. An unreadable
            // database is not this command's problem to diagnose, so it falls through and
            // lets the write report the real error.
            if (key.size() > 5 && key.compare(key.size() - 5, 5, "CA_ID") == 0
                && !args[2].empty()) {
                try {
                    if (!db->get_ca_instance(args[2]).has_value()) {
                        std::cerr << key << ": no CA with id '" << args[2] << "'\n";
                        std::string have;
                        for (const auto& inst : db->list_ca_instances())
                            have += (have.empty() ? "" : ", ") + inst.id;
                        if (have.empty())
                            std::cerr << "  This deployment has no CAs yet — create one first "
                                         "(deployment.md 4.3), then set this.\n";
                        else
                            std::cerr << "  CAs on this deployment: " << have << "\n";
                        return 1;
                    }
                } catch (const std::exception&) {
                    // Cannot read the CA list: do not block the write on it.
                }
            }
