...
#   ./build-check.sh --ref v1.0.0 --release v1.0.0 --package …
#                                    # …and stamp v1.0.0 into the binaries, for publishing
#
# Without --release the binaries report `<commit>-local`, so a hand-built package can never
# be mistaken for a released one. A package that IS being published must pass it.
#
# ── WHY THIS EXISTS ───────────────────────────────────────────────────────────────────
#
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        # ⚠️ WHAT THE BINARIES REPORT AS THEIR VERSION. A local bake labels itself
        # `<commit>-local` on purpose, so a hand-built image can never be mistaken for a
        # released one. A package that is going to be PUBLISHED needs the tag instead:
        # without this, release.yml would ship binaries reporting `<commit>-local` while
        # the container image beside them reported the tag, and every bug report against
        # the native path would name a version no release ever had.
        --release) RELEASE_LABEL="${2:?--release needs a label}"; shift 2 ;;
...
git -C "$ROOT" archive --format=tar "$REF" \
  | docker run -i --name "$NAME" \
        ...
        -e "JOBS=$JOBS" \
        -e "FASTPKI_RELEASE=${RELEASE_LABEL:-$COMMIT-local}" \
        "alpine:$ALPINE" sh -ec '
