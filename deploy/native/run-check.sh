...
i=0; while [ "$i" -lt 30 ]; do
    dexq 'netstat -lnt 2>/dev/null | grep -q ":8090 "' && break
    sleep 2; i=$((i+1))
done
...
i=0; while [ "$i" -lt 20 ]; do
    dexq 'netstat -lnt 2>/dev/null | grep -q ":8080 "' || break
    sleep 2; i=$((i+1))
done
...
# ⚠️ 180s, NOT 40, AND THE NUMBER IS MEASURED RATHER THAN CHOSEN. A settled deployment completes this cycle in
# about five seconds: the gate logs "switched off — stopping", the supervisor respawns the
# process, the new one blocks with its port closed, and it binds within its 10s re-check.
# On the FIRST cycle after an install it takes longer — the install has only just finished,
# the token is cold and the database is still busy behind it — and at 40s this cell reported
# a product failure against a deployment that was working correctly. Measured on a fresh
# bake: a settled container completes the cycle in 5s, while the first cycle after an install
# took longer than 120s — and the process was alive, supervised, with OCSP_ENABLED=true in the
# database, and listening on :::8080 when inspected afterwards. Late, not stuck.
#
# The window is now wide enough to tell LATE from NEVER, which is the distinction that
# matters: never means the gate or the supervisor is broken, late means the host was loaded.
# And when it IS still missing, the log is printed — these services run at LOG_LEVEL=err, so
# the cell used to fail with nothing recorded anywhere, which is not a finding anyone can act
# on.
