...
release's `SHA256SUMS`, unpacks it, installs the Alpine packages the release needs, and then
asks the same questions as a Docker install. Nothing is compiled on your server. Add
`--answers <file>` to answer them from a file, or `--version <tag>` to pin a release instead
of taking the newest.

**If the machine cannot reach GitHub, install from a package instead.** An IPv6-only host
cannot: github.com publishes no IPv6 address. Download the package on a machine that can,
copy it across, and name it:

```sh
sh install.sh --package fastpki-native-<version>-<arch>.tar.gz
```

Take the package matching the machine's processor, and an Alpine release matching the one it
was built for — the installer refuses both mismatches rather than installing programs that
would not start.
