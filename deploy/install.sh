...
#   curl -fsSL $URL | sudo sh -s -- --native              # a native Alpine host
#
# ⚠️ `sh`, AND A STOCK ALPINE HAS NO bash. This script is POSIX sh for that reason — see the
# note at the very top. `bash` still works where it exists; `sh` works everywhere.
#
# ⚠️ AND THE HOST MUST BE ABLE TO REACH github.com, which an IPv6-only one cannot: github.com
# publishes no AAAA record. FastPKI's own cloud module builds IPv6-only nodes, so there the
# piped form cannot work at all. Download the package on a machine that can reach it, copy it
# over, and use --package:
#   sudo ./install.sh --package fastpki-native-v0.2.1-amd64.tar.gz
#
...
# verifies it against SHA256SUMS, unpacks it into / — which is what puts
# fastpki-install-native on the PATH — and hands over to it. It needs root for the unpack.
#
...
if ! in_tree; then
  ...
  # latest release is on the way past. The file has already been checksummed by whoever
  # downloaded it — SHA256SUMS ships beside it — and re-verifying here would need the network
  ...
  if [ -n "$PKG_FILE" ]; then
    ...
    install_runtime_packages || exit 1
  ...
  if [ -z "$V" ]; then
    ...
    # ⚠️ "NO RELEASE" AND "CANNOT REACH GITHUB" ARE DIFFERENT PROBLEMS AND USED TO READ THE
    # SAME. On an IPv6-only host this said the project had no releases, which sent the
    # operator looking at the wrong thing entirely — and IPv6-only is what FastPKI's own
    # cloud module builds. github.com publishes no AAAA record, so such a node cannot reach
    # it at all, however healthy its network is. Measured on a fresh Alpine instance in that
    # VPC and on a running FastPKI node beside it: HTTP 000 after 25 seconds.
    if [ -z "$V" ]; then
      if ! fetch "$API" /dev/null 2>/dev/null; then
        echo "install.sh: cannot reach $API." >&2
        echo "  This host may have no route to it. github.com publishes no IPv6 address, so" >&2
        echo "  an IPv6-only host cannot reach it at all — which is how FastPKI's own cloud" >&2
        echo "  nodes are built." >&2
        echo "  Download the package on a machine that can, copy it over, and install with:" >&2
        echo "    install.sh --native --package /path/to/fastpki-native-<version>-<arch>.tar.gz" >&2
      else
        echo "install.sh: no published release found for $REPO — pass --version <tag>" >&2
      fi
      exit 1
    fi
  fi
  ...
  if [ "$TARGET" = native ]; then
    ...
    # Basename matching, for the same reason as the source tarball below: release.yml
    # generates the sums with `cd dist && sha256sum ./*`, so every line is prefixed `./`.
    nwant=$(sed -n "s#^\([0-9a-f]\{64\}\)[[:space:]]*[*]\{0,1\}\(\./\)\{0,1\}$PKG\$#\1#p" "$TMP/SHA256SUMS" | head -1)
    ngot=$(sha256_of "$TMP/$PKG")
    [ -n "$nwant" ] || { echo "install.sh: SHA256SUMS has no entry for $PKG" >&2; exit 1; }
    if [ "$nwant" != "$ngot" ]; then
      echo "install.sh: CHECKSUM MISMATCH for $PKG" >&2
      echo "  expected $nwant" >&2
      echo "  got      $ngot" >&2
      exit 1
    fi
    echo "== checksum verified (sha256 $(printf '%.8s' "$ngot"))"
    tar xzf "$TMP/$PKG" -C /
    check_alpine_release || exit 1
    install_runtime_packages || exit 1
    command -v fastpki-install-native >/dev/null 2>&1 \
