...
# ── kubectl on another machine ───────────────────────────────────────────────
# ⚠️ THE MACHINE RUNNING THE DEMO USUALLY HAS NO KUBECONFIG FOR THE CLUSTER, while a login on
# one of its nodes is easy to have. K8S_SSH_TARGET in the descriptor names that login, and a
# `kubectl` placed first on PATH runs each call there. It is a file rather than a function
# because dns-auth.sh and dns-cleanup.sh run kubectl too, as separate processes.
#
# Each argument is single-quoted for the remote shell, so a jsonpath, an `sh -c` script or a
# multi-line Corefile arrives as one argument. stdin is forwarded only for `-f -`; every other
# call gets `ssh -n`, so a call cannot swallow input meant for the demo.
demo_kubectl_over_ssh(){
  local bin="$WORK/kubectl-ssh"
  mkdir -p "$bin" || return 1
  {
    echo '#!/usr/bin/env bash'
    echo "# kubectl on $K8S_SSH_TARGET over SSH: written by demo/pki-demo.sh for one run."
    printf 'target=%q\n' "$K8S_SSH_TARGET"
    printf 'opts=(%s)\n' "$(printf '%q ' "${DEMO_SSH_OPTS[@]}")"
    cat <<'SHIM'
cmd=kubectl; n=-n
for a in "$@"; do
  [ "$a" = - ] && n=
  q=$(printf '%sX' "$a" | sed "s/'/'\\\\''/g"); q=${q%X}
  cmd+=" '$q'"
done
exec ssh $n "${opts[@]}" "$target" "$cmd"
SHIM
  } > "$bin/kubectl" && chmod +x "$bin/kubectl" || return 1
...
setup_live(){
  ...
  if [ -n "${K8S_SSH_TARGET:-}" ]; then
    demo_kubectl_over_ssh || { echo "could not set up kubectl over SSH to $K8S_SSH_TARGET" >&2; exit 2; }
  fi
...
ocsp_check(){ # $1=leaf $2=colour $3=label
  ...
  local S; S=$(ocsp_status "$1")
  # ⚠️ "unavailable" IS NOT A STATUS. It printed as a passed step, and the revoke cell below
  # then reported a CMP setting as the cause. Measured: the responder refused every request
  # after its key was replaced, and the demo showed this step green.
  if [ "$S" = unavailable ]; then
    fail "(the OCSP responder did not answer with a status — see its log on the server)"
    return 0
  fi
  ok
...
cmp_revoke_and_confirm(){ # $1=CN of the cmp leaf
  ...
  local st_rr; st_rr=$(ocsp_status cmp-leaf.pem)
  if [ "$st_rr" = revoked ]; then
    REVOKE_OK=yes; ok
    step "re-checking the CMP certificate's status via OCSP"; local S; S=$(ocsp_status cmp-leaf.pem); ok
    printf '     %sstatus:%s %s%s%s\n' "$DIM" "$RST" "$YEL" "$S" "$RST"
  elif [ "$st_rr" = unavailable ]; then
    # ⚠️ NOT A CMP OUTCOME. The rr may well have been accepted; the check that would say so
    # got no answer. This branch used to fall through to the CMP_CLIENT_CA_ID skip below, with
    # CMP_CLIENT_CA_ID set and cmp-rr.log reading "revocation accepted".
    local said; said=$(grep -m1 -iE 'PKIStatus|refus|error' "$WORK/cmp-rr.log" 2>/dev/null | cut -c1-90)
    fail "(the OCSP responder did not answer, so the revocation could not be confirmed${said:+ — the CMP server said: $said})"
  elif grep -q 'revocation accepted' "$WORK/cmp-rr.log" 2>/dev/null; then
    fail "(the CMP server accepted the revocation, but OCSP still answers '$st_rr' — see $WORK/cmp-rr.log)"
...
demo_k8s_client_publish(){   # sets DEMO_K8S_CLIENT_FQDN; 0 on success
  ...
  # ⚠️ macOS HAS NO `ip`. Without this the Service was never created on a Mac, and both
  # cells skipped asking for a CHALLENGE_FQDN. The same answer comes from the interface the
  # route leaves by.
  if [ -z "$src" ] && command -v route >/dev/null 2>&1; then
    iface=$(route -n get "${TARGET_HOST:-1.1.1.1}" 2>/dev/null | awk '/interface:/{print $2; exit}')
    [ -n "$iface" ] && src=$(ifconfig "$iface" 2>/dev/null | awk '$1=="inet"{print $2; exit}')
  fi
  [ -n "$src" ] || return 1
...
have_sudo(){
  ...
  # ⚠️ macOS HAS NO PRIVILEGED PORTS for a listener on every address, which is how both
  # responders bind. Against a remote target nothing else here needs root: /etc/hosts is
  # edited only for the invented local name, and no local compose file is touched.
  if [ "$(uname -s)" = Darwin ] && { [ -n "${CHALLENGE_FQDN:-}" ] || [ -n "${K8S_NAMESPACE:-}" ]; }; then
    demo_sudo=""; return 0
  fi
