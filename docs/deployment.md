...
| **Instrument** | `docker stats --no-stream`, and `du` per volume for the data, read after ten minutes idle |
...
The same script bootstraps itself. Piped, it fetches the release tarball, **checks the release
signature and then the tarball against the signed `SHA256SUMS`**, unpacks it and re-execs
itself from there, so the one-liner and the checkout run identical code:
...
- **The download is checked.** The signature on `SHA256SUMS` is checked first, against the
  FastPKI release key written into the script, and then the tarball against it. A signature
  that does not verify, a release with no signature, or a mismatched hash each stops the
  install before anything is unpacked. There is no option to skip it. The script needs
  `openssl` for the check; on Alpine it installs it itself.
...
| `fastpki_pgdata` | schema, cert inventory, console users, the CA rows in `certs` | **Never.** An update carries the schema forward (see below) |
...
- **Updating from an older release** — every release that changes the schema ships the
  change as a numbered step in `sql/steps/`, and `./rolling-update.sh` applies every step
  newer than the database, in order. So you can update directly from any earlier release,
  without stopping at the versions in between.
- ⚠️ **Wiping `fastpki_pgdata` destroys the deployment's certificates, not just its
  schema.** A CA *is* a row in `certs`, and so is
...
settings never sees them. These 35 have to go in the file or in the database. Several of them
...
⚠️ **That SSH must work without typing a password.** The join carries A's database password and
both token PINs between the hosts on its own connections, so it runs every SSH
non-interactively and a password prompt cannot be answered: with password-only access it stops
at `cannot log in to USER@ADDRESS`. Put your public key on both servers first, which asks for
the password once per server and never again:

```bash
ssh-keygen -t ed25519 -f ~/.ssh/fastpki_ops -C "fastpki ops"    # if you have no key yet
ssh-copy-id -i ~/.ssh/fastpki_ops.pub YOUR-LOGIN@A-ADDRESS
ssh-copy-id -i ~/.ssh/fastpki_ops.pub YOUR-LOGIN@B-ADDRESS
```

Then pass that key as `-i ~/.ssh/fastpki_ops`, or load it into `ssh-agent` and leave `-i` out.
Use addresses **your own computer** can reach; they need not be the addresses the two servers
use between themselves, which are the ones in `PG_BIND`.
...
| **Instrument** | `free -m`, `df -h` and `/proc/loadavg` on the node itself, read after ten minutes idle |
...
```sh
wget -qO- https://github.com/fastpki/fastpki/releases/latest/download/install.sh \
  | sh -s -- --native
```

⚠️ **`wget` and `sh`, not `curl` and `bash`, and on Alpine that matters.** A stock Alpine has
neither `curl` nor `bash`: `wget` is part of busybox and is already there, and the installer
is a POSIX shell script so `sh` runs it. Elsewhere `curl … | bash` works just as well.

That downloads the compiled package for this machine's processor, checks the release
signature and then the package against the signed `SHA256SUMS`, unpacks it, installs the
Alpine packages the release needs, and then asks the same questions as a Docker install.
...
**On an IPv6-only machine, fetch the installer from fastpki.com.** github.com publishes no IPv6
address, so an IPv6-only host cannot reach it at all. fastpki.com serves the same signed
releases over both address families, and the installer falls back to it by itself once it
is running:
...
It checks the same signature whichever site the files came from.

**With no internet at all, copy three files across and install from them.** Download the
package, `SHA256SUMS` and `SHA256SUMS.sig` from the same release, put them in one directory
on the machine, and name the package:

```sh
sh install.sh --package fastpki-native-<version>-<arch>.tar.gz
```

The two small files are what prove the package came from FastPKI, and the check needs no
network; without them the installer refuses. Take the package matching the machine's
processor, and an Alpine release matching the one it was built for — the installer refuses
both mismatches rather than installing programs that would not start.
...
| **Instrument** | `kubectl top pod --containers`, on k3s v1.36.4, read after ten minutes idle |
...
username, and the last write wins. This is true on every deployment path, because every
installer seeds its own `admin` row.

`deploy/mesh-join.sh` checks this before it joins anything. If two data centers each have a
password set for one account it says so, and after the join it names the data center whose
row the mesh kept:

```
mesh-join: WARNING: 'admin' has a password set in more than one data center (1 2).
mesh-join:   These are separate rows for one account. The join keeps one, chosen by write
mesh-join:   order, and the other password stops working. Set it again afterwards, once.
...
mesh-join: 'admin': the mesh kept the row from data center 2 — that password is now the one
mesh-join:   in force everywhere.
```

Setting it again afterwards is one command; [`admin-guide.md`](admin-guide.md) §1.4 has the
form for each deployment path. Directory accounts are unaffected: they are not stored
locally, so there is nothing to collide.
...
(`compatibility.md` §5.1 covers why, and what else this algorithm choice affects). The
...
`deploy/cloud/` runs FastPKI on AWS. Its OpenTofu code creates the network, the disks and one
...
**The servers start from Alpine's own cloud image, and each installs FastPKI at its first
boot.** It downloads the release from fastpki.com, which answers over IPv6 — github.com does
not, and these servers are IPv6-only unless you give them IPv4. Before anything it downloads
is run, it checks the release signature against the public key in your own copy of this
repository (`docs/release-keys/`), so the download site only has to be reachable, not trusted.
Nothing is compiled.

**You can build your own image instead**, with FastPKI already on it — steps 1 and 2. Do that
only when the servers cannot reach the internet at their first boot: such an image downloads
nothing. Everyone else skips straight to step 3.

**You can also skip the OpenTofu code** and launch Alpine's official 3.24 AMI yourself, then
follow §7 on it with the one-line installer from fastpki.com. You then set up yourself
everything the OpenTofu code would: the network, security groups, a second interface for the
data centers to reach each other, a standby, the shared address, the time source, and the
connections between data centers. For a single server that is little work; for a standby or a
mesh, use the OpenTofu code.
...
| Deployment | Steps |
|---|---|
| One server, no standby | 3 to 8. You are finished at step 8. |
| One data center with a standby | 3 to 9, then 12. |
| A mesh, with or without standbys | 3 onwards. |

Add steps 1 and 2 first only if you are building your own image.
...
| **Instrument** | `free -m`, `df -h` and `/proc/loadavg` on the `t3.micro` itself, read after ten minutes idle |
...
- The build takes about 15 minutes on an `m7i-flex.large`. It prints the image id at the end.
  Keep that id: give it to step 3 when it asks for an image. Left empty, step 3 uses Alpine's
  own image and installs FastPKI at first boot instead.
...
them get a standby, the server type, an image id (only if you built one in step 2 — leave it
empty otherwise), the FastPKI release to install (the newest is offered, and pinning it keeps
every server on the same release), the name of your SSH key pair,
...
⚠️ **On a mesh, set the same password on every server.** One `tofu apply` built every node,
and each seeded its own `admin` row in its own database. Once the nodes are joined in step 10
they replicate `admin` as one account, keep one row and discard the rest, so the passwords set
on the other nodes stop working. `deploy/mesh-join.sh` says which row it kept — §9.0 step 2
shows what that looks like — and setting the password again afterwards is one command, in
[`admin-guide.md`](admin-guide.md) §1.4.
...
check the signature on `SHA256SUMS` and then the files against it, unpack them, and run
`deploy/install.sh` from there instead. It is the same code either way.
