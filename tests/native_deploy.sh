...
chk "  the disk image pin is a patch of the AMI pin" yes \
    "$(case "$AIV" in "$AV".*) echo yes ;; *) echo no ;; esac)"
...
for k in DEPLOYMENT PKI_DNS DC_INDEX PG_BIND KEY_BACKEND PKCS11_MODULE \
         PG_PASSWORD PKCS11_PIN CMP_CLIENT_CA_ID; do
