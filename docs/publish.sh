...
# ⚠️ A DOCUMENT SAYS FOR ITSELF THAT IT IS NOT PUBLISHED, and this reads it. The design
# record carries `fastpki:not-published` near its top; nothing else does.
#
# It used to be a list here naming architecture.md, and that is the shape that keeps going
# wrong in this project — two places that can disagree, with nothing to notice when they do.
# Rename the file and the list silently stops matching; the design record is then published
# with everything else and no one finds out until it is on the website. A marker cannot come
# adrift from the file it is in, and a new internal document opts out the same way instead of
# needing somebody to remember this variable exists.
#
# The list is still computed, because publish.lua needs it to turn links to an unpublished
# guide into links to the file on GitHub — but it is derived from the markers, so there is
# one source and the two cannot disagree.
NOT_PUBLISHED_MARKER='fastpki:not-published'
UNPUBLISHED=""
for f in "$HERE"/*.md; do
    head -30 "$f" | grep -q "$NOT_PUBLISHED_MARKER" || continue
    UNPUBLISHED="$UNPUBLISHED ${f##*/}"
done
UNPUBLISHED="${UNPUBLISHED# }"
[ -z "$UNPUBLISHED" ] || echo "==> not published (marked in the file):$UNPUBLISHED" >&2
...
# ⚠️ THE ORDER IS CHOSEN, NOT ALPHABETICAL. Left to the shell glob the list opened with
# admin-guide and api-reference, so the first thing a reader met was the API reference —
# which is the last thing anybody wants when they are deciding whether to deploy FastPKI.
# This is the order somebody actually needs it in: get it running, then run it, then use it,
# then the specialist topics, and the references at the end where you go to look something
# up rather than to learn anything.
GUIDE_ORDER="deployment.md admin-guide.md user-guide.md windows-autoenrolment.md
             authentication.md rbac.md high-availability.md postgres.md manual-procedures.md
             api-reference.md protocol-apis.md cli-reference.md config-reference.md
             components.md compatibility.md"
...
#
# ⚠️ A GUIDE MISSING FROM THE ORDER IS STILL PUBLISHED, at the end. The alternative — only
# what the list names — means adding a guide and forgetting to list it silently drops it
# from the website, which nobody would notice until a reader went looking for it.
GUIDES=""
for b in $GUIDE_ORDER; do
    # ⚠️ A CONTRADICTION IS AN ERROR, NOT SOMETHING TO SKIP QUIETLY. A file that carries the
    # marker AND is named in the order means somebody meant one of the two and did the other.
    # Skipping silently picks an answer for them and hides the mistake.
    case " $UNPUBLISHED " in
        *" $b "*) echo "publish: $b is marked $NOT_PUBLISHED_MARKER and is also in GUIDE_ORDER — decide which" >&2; exit 1 ;;
    esac
    [ -f "$HERE/$b" ] || { echo "publish: $b is in GUIDE_ORDER and does not exist" >&2; exit 1; }
    GUIDES="$GUIDES $b"
done
for f in "$HERE"/*.md; do
    b="${f##*/}"
    case " $UNPUBLISHED " in *" $b "*) continue ;; esac
    case " $GUIDES " in *" $b "*) continue ;; esac
    echo "publish: $b is not in GUIDE_ORDER — publishing it last" >&2
    GUIDES="$GUIDES $b"
done
...
# ⚠️ THE CONTENTS GOES ON EVERY PAGE, BECAUSE prev/next/up IS NOT NAVIGATION. pandoc's
# chunked output links each chapter to its neighbours and nothing else, so a reader who wants
# chapter 9 of a fifteen-chapter guide walks there one page at a time. The PDF has the whole
# contents in front of it; the HTML now does too, down the left.
#
# Built from the generated files rather than from sitemap.json: the filenames carry the
# chapter numbers and each page states its own title, so there is nothing to parse and
# nothing that can disagree with what was actually produced. `sort -V` because a plain sort
# puts 1.10 before 1.2.
build_sidebar() {   # build_sidebar <guide dir> <current file> <guide title>
    printf '<nav class="docs-toc">\n'
    printf '<a class="docs-all" href="../index.html">&#8592; All guides</a>\n'
    printf '<div class="docs-guide-title">%s</div>\n' "$3"
    printf '<ol>\n'
    for _f in $(ls "$1" | grep -E '\.html$' | grep -v '^index\.html$' | sort -V); do
        # ⚠️ READ THE TITLE WITHOUT CARING WHERE THE LINES FALL. pandoc wraps its output, so
        # a long chapter title arrives split across two lines and a line-based match silently
        # misses it — the entry then showed a reader the raw filename,
        # "1.7-granting-a-role-is-bounded-by-what-the-grantor-holds.html". --wrap=none below
        # stops the wrapping; squashing newlines here means this does not depend on that.
        _t=$(tr '\n' ' ' < "$1/$_f" \
             | sed -n 's/.*<title>\(.*\) — FastPKI<\/title>.*/\1/p' \
             | sed 's/  */ /g; s/^ //; s/ $//' | head -1)
        [ -n "$_t" ] || _t="$_f"
        if [ "$_f" = "$2" ]; then
            printf '<li class="here"><a href="%s">%s</a></li>\n' "$_f" "$_t"
        else
...
for g in $GUIDES; do
    ...
    pandoc_run "$PANDOC_IMAGE" \
        -t chunkedhtml --split-level=2 -s --toc --wrap=none \
        --template=publish-page.html \
    ...
    # The guide's own title, from its first heading, for the head of the contents.
    gtitle=$(head -1 "$HERE/$g" | sed 's/^# *//')
    side=$(mktemp)
    for f in "$OUT/$name"/*.html; do
        b="${f##*/}"
        build_sidebar "$OUT/$name" "$b" "$gtitle" > "$side"
        awk -v s="$side" '
            index($0, "@@SIDEBAR@@") { while ((getline l < s) > 0) print l; close(s); next }
            { print }' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
    done
    rm -f "$side"
    # A page that still carries the placeholder was never given a contents, and would show
    # the literal text to a reader.
    if grep -rl '@@SIDEBAR@@' "$OUT/$name" >/dev/null 2>&1; then
        echo "publish: $name has pages with no contents injected" >&2; exit 1
    fi
    printf '    %-28s %s pages\n' "$name" "$n" >&2
done

# The stylesheet the pages link. It travels with them rather than living in the website
# repository, so the guides and the CSS that lays them out are always the same version.
cp "$HERE/docs.css" "$OUT/docs.css"
