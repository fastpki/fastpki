...
public:
    ...
    void session_put(const std::string& token_hash, const SessionRow& s) override {
        ...
        const char* vals[6] = { token_hash.c_str(), s.username.c_str(), s.role.c_str(),
                                mr.c_str(), exp.c_str(), s.groups.c_str() };
        ResPtr r{PQexecParams(conn_,
            "INSERT INTO web_sessions(token_hash,username,role,must_reset,expires,groups) "
            "VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(token_hash) DO UPDATE SET "
            "username=$2,role=$3,must_reset=$4,expires=$5,groups=$6",
            6, nullptr, vals, nullptr, nullptr, 0)};
    ...
    std::optional<SessionRow> session_get(const std::string& token_hash) override {
        ...
        ResPtr r{PQexecParams(conn_,
            "SELECT username,role,must_reset,expires,groups FROM web_sessions WHERE token_hash=$1",
            1, nullptr, vals, nullptr, nullptr, 0)};
