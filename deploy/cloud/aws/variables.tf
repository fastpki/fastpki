...
variable "ami_id" {
  ...
  default     = ""
  description = <<-EOT
    The FastPKI AMI built by deploy/cloud/image (Packer). Leave empty to pick the most
    recent AMI named fastpki-* owned by THIS account.

    ⚠️ There is no public FastPKI AMI to fall back on, and there must not be an unowned
    default here: an AMI id is code that runs as root on a host holding CA keys.
  EOT
}
