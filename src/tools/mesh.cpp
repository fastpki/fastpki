...
const char* kNodeLocalTables =
    ...
    // A device ticket is claimed and verified by the ACME service of the node that issued it:
    // the Apple profile's DirectoryURL names that node, as any ACME client's orders do.
    "acme_device_tickets, "
...
const char* kPublicTables =
    ...
    "client_configs, "
    // The device serial numbers an MDM fleet may enrol by (ACME device attestation). An
    // allow-list an administrator maintains, like allowed_domains: it must hold on every
    // node a device might reach.
    "acme_device_serials";
...
const std::vector<MgmtTable> kMgmtTables = {
    ...
    {"client_configs",      {"kind"}},
    // Registered device serials: admin-authored, keyed by (serial, CA), and the writer
    // stamps `updated`.
    {"acme_device_serials", {"serial", "ca_instance_id"}},
...
void emit_node(const std::vector<Dc>& dcs, const Dc& self) {
    ...
    for (const auto& d : dcs) {
        ...
        // ⚠️ AN EXISTING SUBSCRIPTION TAKES THE TOPOLOGY'S CONNECTION TOO, or a change to it
        // reaches new meshes only. CREATE runs once, so a subscription kept the conninfo it
        // was born with for ever: when each server of a Kubernetes pair moved to its own port,
        // every peer already meshed kept dialling 5432 at both addresses, and would have lost
        // the pair at its first promotion. Set rather than compared: subconninfo is readable by
        // a superuser only, and the native role is not one. Setting an unchanged value restarts
        // the apply worker once, which costs nothing. It runs before REFRESH, which connects to
        // the publisher and should dial the right place.
        std::cout << "SELECT format('ALTER SUBSCRIPTION %I CONNECTION %L', " << sql_lit(sub) << ",\n"
                     "              " << sql_lit(d.conninfo) << ")\n"
