...
# ⚠️ STAMP THE RELEASE INTO THE BINARIES, NOT JUST INTO THE IMAGE NAME. The source arrives
# here as a tarball with no .git, so CMake's `git describe` fallback finds nothing and every
# binary reported 0.0.0-dev — the AMI name was the only record of what the image held, and a
# node could not answer "which build am I running?" at all. Packer already hands us the
# release string; it is the same value the image is named for, so the two now agree.
FASTPKI_VERSION="${FASTPKI_RELEASE:-}" \
