...
envsubst < "$MANIFESTS/service-protocols.yaml" | kubectl apply -n "$NAMESPACE" -f -
...
# Compose pair.
if [ "${PG_EXTERNAL_TYPE:-none}" != "none" ] || [ "${P11_TLS_SERVICE_TYPE:-ClusterIP}" != "ClusterIP" ]; then
    _i=0
    while [ "$_i" -lt "$NODE_COUNT" ]; do
        NODE_ORDINAL=$_i
        export NODE_ORDINAL
        if [ "${PG_EXTERNAL_TYPE:-none}" != "none" ]; then
            ...
            envsubst < "$MANIFESTS/postgres-external.yaml" | kubectl apply -n "$NAMESPACE" -f -
        fi
        if [ "$P11_TLS" = "on" ] && [ "${P11_TLS_SERVICE_TYPE:-ClusterIP}" != "ClusterIP" ]; then
            export P11_TLS_SERVICE_TYPE
            envsubst < "$MANIFESTS/p11-tls-external.yaml" | kubectl apply -n "$NAMESPACE" -f -
        fi
        _i=$((_i + 1))
    done
fi
...
echo "Protocols: web($WEB_PORT) ocsp(8080) est(8443) acme(8444) cmp(8445) ms(8446) store(8447) scep(8448)"
...
if [ -n "${PG_INTERCONNECT:-}" ]; then
    ...
    echo "  as PG_TLS_SANS."
    # The topology, its anchor path inside the postgres container (/pki, not the apps' /var/pki)
    # and both passes are deploy/mesh-join.sh's job, the same command on every deployment path.
