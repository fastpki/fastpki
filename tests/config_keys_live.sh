...
# ⚠️ THESE ASSERTIONS ONLY MEAN SOMETHING WHILE SCHEMA STEPS EXIST. The 39 steps that
# carried the schema to its current shape were collapsed into sql/createdb.sql: they only
# ever applied to a database that already existed, and there are none. The guard stays
# armed for the next real step rather than being deleted — but it must not report a pass
# over an empty set, which is how a check quietly stops checking.
if [ -d "$ROOT/sql/steps" ] && ls "$ROOT/sql/steps"/[0-9]*.sql >/dev/null 2>&1; then
  ...
  # Anti-vacuity: if the extraction ever stops matching, the assertion above passes over an
  # empty set forever. Steps DO read config keys today, so zero means the pattern rotted.
