...
Provision FastPKI in a public cloud: a custom Alpine machine image with the binaries and
the patched PKCS#11 stack installed in it, and infrastructure-as-code that stands up the network,
the instances and the mesh.
...
packer build -only=fastpki.amazon-ebs.alpine \
    -var 'release=1.2.3' deploy/cloud/image            # the AMI, once per release
...
The split is the same as AWS's. Packer builds the image; the IaC only consumes one. There
the artifact is an AMI, here it is a Proxmox **template** the module clones:
...
full toolchain on every production host and add ten minutes to each launch. Building them
once, in the image, is the whole argument: an instance boots with the patched stack in
place, and cloud-init only answers the deployment questions.
