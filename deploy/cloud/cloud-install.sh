...
#
# A member of the install.sh family, and it asks the SAME questions as the others in the same
# order with the same defaults:
#
#   deploy/install.sh                 a docker-compose deployment on a host you have
#   deploy/k8s/k8s-install.sh         a Kubernetes cluster you have
...
if [ "$DESTROY" = 1 ]; then
  ...
  # A standby's volume carries prevent_destroy too, so it is excluded as well; left out, a
  # deployment with a standby could not be torn down at all.
  "$TF" destroy -exclude="aws_ebs_volume.data" -exclude="aws_ebs_volume.standby_data"
  # ⚠️ THEN FORGET THE KEPT VOLUMES. Their database password was on the system disk, which is
  # gone, so no new server can use them. Left in the state, the next install from this
  # directory attaches them to its new servers, which then refuse to start. Removing them
  # from the state leaves them untouched in AWS and lets the next install create new ones.
  kept=""
  for addr in $("$TF" state list 2>/dev/null | grep -E '^aws_ebs_volume\.(data|standby_data)\['); do
    vol="$("$TF" state show "$addr" 2>/dev/null | sed -n 's/^ *id *= *"\(vol-[0-9a-f]*\)".*/\1/p' | head -1)"
    "$TF" state rm "$addr" >/dev/null || die "could not remove $addr from the state"
    kept="$kept ${vol:-$addr}"
  done
  echo >&2
  if [ -n "$kept" ]; then
    echo "Kept in AWS, unattached and still billed:$kept" >&2
    echo "They hold every issued certificate and, with SoftHSM, every CA private key, but no" >&2
    echo "server can start from them again. The next install creates new volumes." >&2
    echo "Delete one once you are certain:  aws ec2 delete-volume --volume-id <id>" >&2
  fi
  exit 0
fi
...
# separate machine joined to a running server (docs/high-availability.md §3), so answering
...
echo "  add later (docs/high-availability.md §3) — and its CA keys must be created" >&2
echo "  replicable from the start, because that cannot be granted afterwards." >&2
# Several names mean one per data center, so their number IS the count and the question is
# not asked. One name may front one data center or several (a shared name), so it is.
pki_dns_count=$(printf '%s' "$PKI_DNS" | tr ',' '\n' | grep -c '[^[:space:]]' || true)
if [ "$pki_dns_count" -gt 1 ]; then
  _given="$(ansval DC_COUNT)"
  [ -z "$_given" ] || [ "$_given" = "$pki_dns_count" ] \
    || die "the answers file sets DC_COUNT=$_given but gives $pki_dns_count names — one name per data center"
  DC_COUNT="$pki_dns_count"
  echo "  $pki_dns_count names given: $pki_dns_count data centers, one name each." >&2
else
...
# Blank is the normal answer: Alpine's own image, with FastPKI installed at first boot after
# the release signature is checked. An image built with deploy/cloud/image is for servers
# that cannot reach the internet when they first boot.
ask AMI_ID "Image with FastPKI already on it (blank = Alpine's own image, installed at first boot)" ""

# ⚠️ PINNED HERE, SO EVERY SERVER OF THIS DEPLOYMENT INSTALLS THE SAME RELEASE. Left blank,
# each server takes whatever is newest at its own first boot, so a standby or data center
# added months later would run a newer release than the servers already there. The newest
# release is offered as the default; blank still means "newest at first boot".
FASTPKI_VERSION_DEFAULT=""
if [ -z "$AMI_ID" ]; then
  FASTPKI_VERSION_DEFAULT=$( { curl -fsS --connect-timeout 10 https://fastpki.com/releases/latest 2>/dev/null \
      || wget -qO- -T 10 https://fastpki.com/releases/latest 2>/dev/null; } | tr -d ' \r\n' ) || true
  ask FASTPKI_VERSION "FastPKI release to install (blank = the newest at each server's first boot)" "$FASTPKI_VERSION_DEFAULT"
else
  FASTPKI_VERSION=""
fi
...
ask CLIENT_CIDRS "Who may reach the enrolment protocols and the console, to enrol devices (blank = nobody outside the VPC)" ""
...
# The questions below are install.sh's, in its order: key storage, the protocols, each
# service's key, the SCEP RA key size, and last the module of your own HSM. Each node's
# install-native.sh receives the answers unchanged.
...
# Every answer here lands in service_keys, which each node writes into its own answers file.
SERVICE_KEYS=""
# ⚠️ `[ -z ] ||`, not `[ -n ] &&`: under `set -e` the second form returns non-zero for a blank
# answer — a digest left on auto — and the wizard would stop there.
keep_key() { [ -z "$2" ] || SERVICE_KEYS="${SERVICE_KEYS:+$SERVICE_KEYS, }$1 = \"$2\""; }
...
if [ "$WANT_SCEP" = yes ]; then
  ...
  keep_key SCEP_RA_KEY_BITS "$SCEP_RA_KEY_BITS"
fi

PKCS11_MODULE=""; PKCS11_TOKEN="fastpki"
if [ "$KEY_BACKEND" = hsm ]; then
