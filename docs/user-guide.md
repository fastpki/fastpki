...
(Safari on an iPhone or iPad), choose the CA under **Files enrol against**, and press **Apple
ACME profile**. The profile carries a new one-time ticket issued to you, the CA's root and
the device name `<your-username>-device`. Install it, and the device enrols by itself. Each
download enrols one device. The rest of this section shows what the profile contains, for an
administrator preparing one by hand or for an MDM.
