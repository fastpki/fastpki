...
#
# ⚠️ BY DEFAULT FROM ALPINE'S OWN IMAGE, AND FASTPKI IS INSTALLED AT FIRST BOOT. This used to
# require an image the operator had built with Packer, because the patched PKCS#11 stack had to
# be compiled somewhere and compiling at every boot meant a toolchain on every CA host. Releases
# now publish that stack prebuilt in the native package, so first boot downloads it, checks the
# release signature, and unpacks it — nothing is compiled. The image build became a fifteen-
# minute step that bought nothing but a faster first boot, and it was the one step no other
# deployment path asks for.
#
# ami_id still takes an image built with deploy/cloud/image, for a deployment whose servers may
# not reach the internet at first boot: that image carries everything and downloads nothing.
# First boot tells the two apart by whether fastpki-install-native is already on the disk.
#
# The architecture comes from the instance type, so a Graviton type gets the aarch64 image and
# an Intel or AMD type the x86_64 one without a second setting that could disagree with it.
...
data "aws_ami" "alpine" {
  ...
  # ⚠️ THE OWNER IS THE SUPPLY-CHAIN CONTROL. The same name filter without it also returns a
  # third-party seller's builds named alpine-*, and nothing in the name tells them apart.
  # 538276064493 publishes Alpine's official cloud images; deploy/cloud/image/fastpki.pkr.hcl
  # records how that was verified.
  owners      = [var.alpine_ami_owner]
  most_recent = true
  filter {
    # uefi: published for both architectures. cloudinit: `tiny` has no cloud-init and so
    # cannot be handed its first-boot script. `-r*` keeps out the bare-metal variant, which
    # otherwise ties on creation time and turns the choice into a coin flip.
    name   = "name"
    values = ["alpine-${var.alpine_version}.*-${local.alpine_arch}-uefi-cloudinit-r*"]
  }
  filter {
    name   = "state"
    values = ["available"]
...
locals {
  alpine_arch = contains(data.aws_ec2_instance_type.node.supported_architectures, "arm64") ? "aarch64" : "x86_64"
  ami_id      = var.ami_id != "" ? var.ami_id : data.aws_ami.alpine[0].id
  # What first boot installs when the image does not already carry FastPKI. The public key is
  # read from this checkout, so the signature a node checks is anchored in code the operator
  # already has, never in anything it downloads.
  release_pubkey = file("${path.module}/../../../docs/release-keys/release-ecdsa.pub")
...
resource "aws_instance" "node" {
  ...
  user_data_base64 = base64gzip(templatefile("${path.module}/user-data.sh.tftpl", {
    ...
    host_name            = "${var.deployment_name}-${each.value.index}"
    ...
    pkcs11_token         = var.pkcs11_token
    service_keys         = var.service_keys
    ...
    # Used only when the image does not already carry FastPKI (see the top of this file).
    release_version = var.fastpki_version
    release_mirror  = var.release_mirror
    release_pubkey  = local.release_pubkey
    # One script for both cloud first boots, so the signature check exists once.
    firstboot_install = file("${path.module}/../firstboot-install.sh")
  }))
...
# See the standby_join output and docs/high-availability.md §3.
...
resource "aws_instance" "standby" {
  ...
  # native procedure does too — docs/manual-procedures.md §11 (Step 2) replaces the database it
  ...
  user_data_base64 = base64gzip(templatefile("${path.module}/user-data.sh.tftpl", {
    dc_index             = each.value.index
    host_name            = "${var.deployment_name}-${each.value.index}-standby"
    dc_count             = var.dc_count
    pki_dns              = length(var.pki_dns) == 1 ? var.pki_dns[0] : var.pki_dns[each.value.index - 1]
    key_backend          = var.key_backend
    pkcs11_module        = var.pkcs11_module
    ...
    web_port             = var.web_port
    ha_enabled           = true
    # The same setting as its primary. With fastpki_version empty each server resolves the
    # newest release at its own first boot, so a standby added later can install a newer one
    # than its primary: pin fastpki_version for a deployment that will grow.
    release_version = var.fastpki_version
    release_mirror  = var.release_mirror
    release_pubkey  = local.release_pubkey
    # One script for both cloud first boots, so the signature check exists once.
