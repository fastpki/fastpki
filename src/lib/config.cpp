...
bool apply2(Config& c, const std::string& key, const std::string& val) {
    ...
    else if (key == "ACME_ATTESTATION_ROOTS") c.acme_attestation_roots = val;
    else if (key == "ACME_ATTEST_MIN_OS") c.acme_attest_min_os = val;
    else if (key == "ACME_ATTEST_REQUIRE_SIP") c.acme_attest_require_sip = (val == "1" || val == "true" || val == "yes");
...
void apply3(Config& c, const std::string& key, const std::string& val) {
    ...
    else if (key == "TRUSTED_PROXIES")    c.trusted_proxies = split_csv(val);
    ...
    else if (key == "LICENSE")                c.license_text = val;
    else if (key == "LICENSE_FILE")           c.license_file = val;
...
Config Config::from_env() {
    ...
    for (const char* k : {
        ...
        "ACME_ATTESTATION_ROOTS","ACME_ATTEST_MIN_OS","ACME_ATTEST_REQUIRE_SIP",
