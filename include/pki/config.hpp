...
struct Config {
    ...
    // Extra trust anchors for ACME device attestation (device-attest-01), beside the Apple
    // Enterprise Attestation Root that is compiled in: PEM certificates, or the same PEM
    // base64-encoded on one line so it fits a config file. It ADDS roots and cannot remove
    // Apple's or skip a check — every attestation is still verified in full.
    std::string acme_attestation_roots;
    // Posture checks on an attested Apple device. Each only ADDS a refusal; neither can pass
    // a device the attestation would otherwise refuse. Off (empty / false) by default.
    //   acme_attest_min_os: refuse an attested OS version below this ("26.0"). A device that
    //     attests no version (older than iOS 17.2 / macOS 14.2) is refused while it is set.
    //   acme_attest_require_sip: refuse a Mac whose attestation says SIP is off. An
    //     attestation with no SIP value (iPhone, iPad, or a Mac too old) passes this check.
    std::string acme_attest_min_os;
    bool acme_attest_require_sip{false};
    ...
    // The signed licence we issued, either inline or as a path to the file.
    //
    // LICENSE holds the file's TEXT and is what the console's upload writes, so a licence can
    // be installed from a browser with nothing to copy onto the server — which matters most
    // on Compose and Kubernetes, where putting a file on the node means a volume mount. It
    // lives in the `config` table, so on a mesh it reaches every node by replication and is
    // uploaded once.
    //
    // LICENSE_FILE points at a file instead, for a deployment that would rather mount one (a
    // Kubernetes Secret, say). LICENSE wins when both are set.
    //
    // Empty both and the built-in evaluation applies — see include/pki/license.hpp for the
    // format, the evaluation, and why none of it enforces anything.
    std::string license_text;            // LICENSE — the licence file's text
    std::string license_file;            // LICENSE_FILE — a path to it instead
