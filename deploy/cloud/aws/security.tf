locals {
  ...
  # cidrsubnet(c, 0, 0) writes each range the way AWS stores it: 2001:db8:0f0f::/64 comes
  # back as 2001:db8:f0f::/64, and a rule written the other way was "updated" on every plan.
  admin_v4  = [for c in var.admin_cidrs : cidrsubnet(c, 0, 0) if !strcontains(c, ":")]
  admin_v6  = [for c in var.admin_cidrs : cidrsubnet(c, 0, 0) if strcontains(c, ":")]
  client_v4 = [for c in var.client_cidrs : cidrsubnet(c, 0, 0) if !strcontains(c, ":")]
  client_v6 = [for c in var.client_cidrs : cidrsubnet(c, 0, 0) if strcontains(c, ":")]
  # Who may reach the enrolment protocols and revocation: administrators plus clients.
  # distinct() because the two lists often share a CIDR (one office is both), and the
  # protocol rules key on "<protocol>-<cidr>", so a repeat is a "Duplicate object key" error.
  serving_v4 = distinct(concat(local.admin_v4, local.client_v4))
  serving_v6 = distinct(concat(local.admin_v6, local.client_v6))
}
...
resource "aws_vpc_security_group_ingress_rule" "console" {
  # Administrators AND clients: users download their device profiles from the console, which
  # requires a login and checks roles. SSH, above, stays with administrators only.
  ...
  description = "FastPKI console (HTTPS) from an administrator or a client"
}
...
resource "aws_vpc_security_group_ingress_rule" "console_v6" {
  ...
  to_port           = var.web_port
  ip_protocol       = "tcp"
  description       = "FastPKI console (HTTPS) from an administrator or a client (IPv6)"
}
