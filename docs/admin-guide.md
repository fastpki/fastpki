...
**Sessions** end after 15 minutes without any action, and at the latest 12 hours after
sign-in. When a session ends, the console shows the sign-in form with the message "Your
session has ended". Sign in again to carry on. To sign out, use
...
![The administrator Dashboard](images/admin-dashboard.png)
...
- **The key algorithm.** RSA and EC work with every client. Ed25519, Ed448 and the ML-DSA
  (post-quantum) algorithms do not: Windows and macOS cannot validate a chain from such a
  CA, and PostgreSQL cannot use a certificate from one. RSA-PSS works everywhere except
  macOS and iOS, which reject everything below an RSA-PSS CA. Changing a CA's algorithm later means creating
...
![The CAs page: a disabled root and two issuing CAs](images/admin-cas.png)
...
![The New CA form](images/admin-ca-create.png)
...
![The Import an existing CA form](images/admin-ca-import.png)
...
![Create CSR (key in HSM)](images/admin-ca-create-csr.png)
...
![Request from a CSR, signing a CA request](images/admin-ca-request-from-csr.png)
...
![The HSM keys page: every key in the token and what it serves](images/admin-hsm-keys.png)
...
![The Inventory: every certificate the deployment issued](images/admin-certificates.png)
...
![The New user form](images/admin-new-user.png)
...
![Editing a user, with the enrolment credentials hidden](images/user-edit.png)
...
![The New group form](images/admin-new-group.png)
...
![Import groups from LDAP](images/admin-import-groups-from-ldap.png)
...
![Import user from LDAP](images/admin-import-user-from-ldap.png)
...
**Device tickets for Apple devices.** A Mac, iPhone or iPad enrolling over ACME with device
attestation has no external account key. It uses a one-time ticket instead. Users get one
inside the Apple ACME profile they download from their Dashboard; you issue one for someone else
on the **Enrolment codes** page (§6.12), or on the command line:

```bash
fastpki-acme --config /etc/fastpki/bootstrap.conf \
    --issue-device-ticket --ca issuing-ca --owner alice [--profile <name>] [--ttl <seconds>]
```

The ticket goes into the device's ACME configuration profile as `ClientIdentifier`
([user-guide.md](user-guide.md) §6.5). It enrols one device, once, against that CA; the
certificate is issued to `alice`, under `<name>` or under her roles' profiles. It expires after
7 days unless `--ttl` says otherwise. The command refuses when the CA does not exist, when
`alice` holds no `acme:enrol` for it, or when the profile does not apply to her. The command
above is for a native or cloud node. On the other paths it runs inside the ACME container:

```bash
# Docker Compose, from deploy/
docker compose exec acme fastpki-acme --config /app/config/bootstrap.conf --issue-device-ticket …
# Kubernetes
kubectl -n fastpki exec statefulset/fastpki-node -c acme -- \
    fastpki-acme --config /app/config/bootstrap.conf --issue-device-ticket …
```
...
![The Computers page](images/admin-computers.png)
...
### 6.12 The Enrolment codes page

![The Enrolment codes page: device tickets, device serials and SCEP challenges](images/admin-enrolment-codes.png)

The codes a device enrols with when it has no account key of its own. It needs `user:manage`,
like the rest of this chapter, and its forms are hidden while console writes are off. A code is
shown in full only while it is unused; after that, only its first 8 characters.

**ACME device tickets.** A ticket lets one Mac, iPhone or iPad enrol once over ACME with device
attestation ([user-guide.md](user-guide.md) §6.5). Choose the **CA**, type the **Owner** (the
user the certificate is issued to), optionally a **Profile**, and how many days it stays valid
(7 by default). **Issue ticket** shows the ticket to hand over; **Issue and download Apple
profile** gives you the finished profile for that owner, to send by email or AirDrop. The list
shows each ticket's state (unused, in progress, used or expired) and, once used, the serial
number the device attested and the certificate it got. **Cancel** stops an unused ticket at
once.

**Registered device serials (MDM).** An MDM platform builds one profile for a fleet and fills in
per-device values. Put the MDM's serial-number variable in the profile's `ClientIdentifier` (in
Jamf, `$SERIALNUMBER`), and list the devices here. A listed device enrols against its CA with no
ticket; Apple signs the serial in the attestation, so a device cannot claim another's. Add one
with **Serial**, **CA**, **Owner** and optional **Profile**, or paste a CSV exported from the MDM
and press **Import CSV**: one `serial[,owner[,profile]]` per line, the fields above filling in
what a line leaves out, `#` lines skipped. Each line is checked the way a ticket is, and the
ones refused are listed with the reason. The list replicates to every data center. **Remove**
takes a device off it. A device enrolled through User Enrollment does not attest its serial
number, so it cannot use this path.

Whichever way a device enrols, its next certificate revokes its previous one as superseded,
matched by the attested serial number.

**SCEP one-time challenges.** A challenge password that one SCEP enrolment consumes, issued to
the SCEP identity (`scep`) and optionally bound to a **Profile**. It is refused when the `scep`
identity has no profile that applies. The SCEP service accepts these challenges only while
`SCEP_DYNAMIC_CHALLENGE` is on, and the page says so when it is off.

On the command line: `fastpki-acme --issue-device-ticket` (§6.7) and `fastpki-scep
--issue-challenge` ([cli-reference.md](cli-reference.md)). The API is
`/api/device-tickets`, `/api/device-serials` and `/api/scep-challenges`
...
![The Directories page](images/admin-directories.png)

### 7.2 Adding an LDAP or Active Directory directory

![Adding an LDAP or Active Directory directory](images/admin-add-directory.png)
...
![Adding a SAML provider](images/admin-add-provider.png)
...
![The Profiles page](images/admin-profiles.png)

![Editing a policy profile](images/admin-profile-editor.png)
...
| **Device serial in the certificate** | tick to add the serial number Apple attested to the SubjectAltName, as an RFC 4043 permanentIdentifier. Only certificates issued through ACME device attestation get it. Useful where a RADIUS server or another relying party matches devices to the MDM inventory; leave it off for certificates shown to arbitrary servers, which would otherwise learn the serial |
...
![The Templates page](images/admin-templates.png)
...
![The certificate template editor](images/admin-new-template.png)
...
![Import templates from AD (CSV)](images/admin-import-templates-from-ad-csv.png)
...
![The Domains page](images/admin-approved-domains.png)
...
![The Client configs page](images/admin-client-configs.png)

This page offers six files that point a client straight at this deployment:
...
| **Apple ACME profile** | `fastpki-acme.mobileconfig` | a Mac, iPhone or iPad: ACME with device attestation |
| **Apple SCEP profile** | `fastpki-scep.mobileconfig` | a Mac, iPhone or iPad: SCEP |
...
**The Apple profiles are opened by the device itself.** On an iPhone or iPad, sign in to the
console in Safari and tap the button; Safari offers the profile, which is installed under
**Settings → Profile Downloaded**. On a Mac, the file downloads and installs from **System
Settings → General → Device Management**. Both carry the CA's root and name the device
`<username>-device`. Each download of the **ACME** profile issues a new one-time device ticket
to the downloading user, recorded in the audit log as `acme_device_ticket_issued`, so one
download enrols one device. The download is refused, with the reason shown in the browser,
when the user's roles do not allow ACME (or, for the SCEP profile, SCEP) enrolment against the
chosen CA.

**The console signs both Apple profiles** with its own TLS certificate, the one the device just
connected to, so iOS and macOS show who made the profile and refuse a copy that was changed on
the way. The profile shows as **Verified** once the device trusts your root, and as **Not
Verified** before that; it installs the same way in both cases. A console serving plain HTTP
has no certificate to sign with and serves the profile unsigned, saying so in its log.
...
| `{{CA_ID}}`, `{{ROOT_CA_DER_BASE64}}` | the chosen CA's id, and its root certificate in base64 DER |
| `{{DEVICE_TICKET}}` | a new one-time device ticket for the downloading user, issued only when the file contains this token |
| `{{DEVICE_CN}}` | the device's name, `<username>-device` |
| `{{UUID_1}}`, `{{UUID_2}}`, `{{UUID_3}}` | new UUIDs for a profile's payloads |
...
![The Endpoints page: each protocol, its addresses and whether it is running](images/admin-endpoints.png)
...
![The Config page, Certificate Authority area](images/admin-config-ca.png)
...
![The audit log](images/admin-audit-log.png)
...
⚠️ **Behind a load balancer or a reverse proxy, set `TRUSTED_PROXIES` or every entry records
the proxy.** The source address on an entry is the address FastPKI saw on the connection. If
something sits in front, that is the proxy's address, identically for every client, and the
log cannot say who did anything. Naming the proxy in `TRUSTED_PROXIES` makes FastPKI read the
client's own address from the `X-Forwarded-For` the proxy sets. Name only proxies you run:
[`config-reference.md`](config-reference.md) explains why, and what the setting does to the
sign-in backoff as well.
...
![The Notifications page](images/admin-notifications.png)
...
![The Discovered page: certificates found by scanning](images/admin-discovered.png)
...
![The Compliance page](images/admin-compliance.png)
...
![The Replication page of an HA pair](images/admin-replication.png)
...
sync every host's nightly job runs (high-availability.md §4), started now instead of tonight. The
...
![The Backup page](images/admin-backup.png)
...
![The Version page, on the evaluation licence](images/admin-version.png)

The **Version** page shows the running version and this deployment's licence, which reads one
of five ways:

| | |
|---|---|
| **Evaluation** | the built-in 30 days, and the date they end |
| **Evaluation ended** | the 30 days have passed |
| **Licensed** | the number, who it was issued to, the tier, the node count, and either "perpetual" or the expiry date |
| **Expired** | the same, past its expiry date |
| **Licence not valid** | the configured file did not parse, or its signature did not verify — and why |

A new install starts in **Evaluation**: every build carries a 30-day licence, and the node
counts from the day it first ran.

**To install the licence we issue you**, use the **Install licence** button on this page:
choose the file, or paste it into the box, and press it. The console checks the signature
before storing it, so a file that was pasted wrong is refused immediately rather than leaving
the deployment in a "not valid" state you then have to diagnose. **Remove** puts it back on
the evaluation.

Nothing is copied onto the server: the licence is stored in the `config` table, so on a mesh
it reaches every node by replication and you upload it once. From a script, the same thing is
`fastpki-config set LICENSE "$(cat acme.license)"`. A deployment that would rather mount the
file — a Kubernetes Secret, say — can point `LICENSE_FILE` at a path instead.

FastPKI is free for non-commercial use under PolyForm Noncommercial 1.0.0, for ever. The
evaluation is for trying it in a **commercial** setting; when it ends, non-commercial use
carries on unaffected, and the page says so.

Nothing enforces any of this. FastPKI does not refuse to start, limit issuance, turn a feature
off, stop working after the expiry date, or contact us. Every field is reported and acted on
nowhere. Unless a licence is in force the console also shows a marker beside the navigation on
every page — somebody who has paid never sees it — and every service writes one line about the
licence to its log at startup.

`config-reference.md` has the file format and the command to check its signature yourself.

**Check for updates** asks the release feed (GitHub's
...
when an update is available, 0 when up to date and 1 when the check failed.

To check a download, verify the release's `SHA256SUMS` first and then the files against it.
The signature on `SHA256SUMS` is what says the files came from us; the hashes alone only catch
a corrupted download. `fastpki-update` carries the release keys, so it needs no setup:

```bash
fastpki-update verify SHA256SUMS SHA256SUMS.sig
sha256sum -c --ignore-missing SHA256SUMS
```

`SHA256SUMS.mldsa.sig` is the same signature made with a post-quantum key, and either one
passing is enough. `docs/release-keys/` has the commands for checking with `openssl` instead.
...
updated from a package built somewhere else.

**Every release publishes that package**, one per processor architecture, beside the source
and the container images:

```
fastpki-native-<version>-amd64.tar.gz
fastpki-native-<version>-arm64.tar.gz
```

Download the one matching the node — an arm64 node needs the arm64 package — together with the
release's `SHA256SUMS` and `SHA256SUMS.sig`, check them, and copy all three to the node:

⚠️ **It must also match the node's Alpine release.** Alpine moves shared-library versions
between releases, so a package built on one and unpacked on another installs cleanly and then
fails to start anything. The package records what it was built on, and the installer stops
with a message naming both rather than leaving a node that reports success and serves
nothing. `cat /etc/alpine-release` on the node says which you need.

```bash
B=https://fastpki.com/releases/<version>     # or https://github.com/fastpki/fastpki/releases/download/<version>
curl -fsSLO $B/fastpki-native-<version>-<arch>.tar.gz
curl -fsSLO $B/SHA256SUMS
curl -fsSLO $B/SHA256SUMS.sig
openssl dgst -sha256 -verify docs/release-keys/release-ecdsa.pub -signature SHA256SUMS.sig SHA256SUMS
sha256sum -c --ignore-missing SHA256SUMS
```

The signature first: `SHA256SUMS` comes from the same place as the package, so on its own it
catches a corrupted download and not a substituted one. `docs/release-keys/release-ecdsa.pub`
is in your copy of the repository. fastpki.com serves the same files as GitHub and, unlike
GitHub, answers over IPv6, so an IPv6-only node can download them itself.

Build it yourself only when you are updating to something that has no release — a branch, or a
fix you are carrying locally. *Building the package for cloud nodes*, below, is how.

On each node, as root:

1. Install the new programs and PKCS#11 components over the old ones:
   - **Installed from a release — a cloud node, or a native install made with the one-line
     installer**: once the two checks above pass, `tar xzf fastpki-native-<version>-<arch>.tar.gz -C /`.
   - **Installed from a source checkout**: put the new release's source tree on the node and
     run `sh deploy/native/build-native.sh`.
2. Run the installer again with the same answers:
   ...
   programs. On an HA pair it also keeps the database host list, so the pair still fails over
   after the update.

   On the **standby** of an HA pair it says

   ```
   ==> this server is a standby — its database replicates from the primary, so the
       role, schema and seed steps are the primary's and are skipped here
   ```

   and restarts the services, which is the whole of what an update needs there. Update the
   standby first, then the primary. The services
...
```bash
sh deploy/native/build-check.sh --ref <release tag> --package fastpki-native-<version>-<arch>.tar.gz
```
...
The installer checks that the patched libraries are the ones on disk, and stops if one has been
replaced:

```
install-native: Alpine's stock p11-kit or SoftHSM has replaced the patched build this package installed (/usr/lib/libp11-kit.so.0 ) — unpack the FastPKI package again (tar xzf fastpki-native-<version>-<arch>.tar.gz -C /), then run this again
```

Do what it says: unpack the same package again and re-run the installer. Nothing else changes.

If the console reports Ed25519, Ed448 or ML-DSA keys as not in the token on a native or cloud
server, this is the cause. Run steps 1 and 2 above with the package the server already runs; the
services restart with the patched libraries.
...
over, and update the other host. The steps are [high-availability.md](high-availability.md) §8.
