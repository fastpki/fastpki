...
struct IssuanceInput {
    ...
    // The serial number Apple attested for the device this certificate is for (ACME
    // device attestation). When the profile sets `device_serial_san`, it is added to the
    // SubjectAltName as an RFC 4043 permanentIdentifier. Empty on every other path, where
    // nothing attests a serial and a serial from a CSR would be whatever was typed.
    std::string attested_serial{};
};
...
X509Ptr issue_cert_from_parts(const Config& cfg, X509* ca_cert, EVP_PKEY* ca_key,
                              ...
                              // Added as a permanentIdentifier SAN when the profile asks
                              // (IssuanceInput::attested_serial).
