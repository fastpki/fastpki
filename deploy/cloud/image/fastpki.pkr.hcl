...
variable "instance_type" {
  ...
  description = <<-EOT
    ...
    ⚠️ AN ACCOUNT ON THE AWS FREE TIER PLAN CANNOT LAUNCH THIS TYPE AT ALL. That plan caps
    which instance types the account may run, not how many hours it gets, so the launch is
    refused before anything is built:

      InvalidParameterCombination: The specified instance type is not eligible for Free Tier.

    List what such an account may use with
    `aws ec2 describe-instance-types --filters Name=free-tier-eligible,Values=true`. The
    largest is m7i-flex.large (x86_64, 2 vCPU, 8 GB), which is enough: 8 GB gives
    build-native.sh's two parallel jobs room, and a measured end-to-end build — compile,
    stop, snapshot and register — took 13m45s. So the 2-vCPU gamble above is about MEMORY
    rather than cores; it is lost on a 1-2 GB box, not on this one.
...
source "amazon-ebs" "alpine" {
  ...
  # Packer's temporary security group defaults to 0.0.0.0/0, and this build is a quarter hour of
