...
#   curl -fsSL $URL | sudo bash -s -- --native            # a native Alpine host
#
...
if ! in_tree; then
  ...
  if [ "$TARGET" = native ]; then
    ...
    if [ -n "$ANSWERS" ]; then
      ...
      NARGS+=(--answers "$ANSWERS")
    fi
    echo "== unpacked; handing over to fastpki-install-native"
    exec fastpki-install-native ${NARGS[@]+"${NARGS[@]}"}
  fi
...
if [ "$TARGET" = native ]; then
  ...
  NARGS=()
  [ -n "$ANSWERS" ] && NARGS+=(--answers "$ANSWERS")
  exec "$HERE/native/install-native.sh" ${NARGS[@]+"${NARGS[@]}"}
fi
