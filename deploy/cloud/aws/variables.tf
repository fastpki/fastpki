...
variable "standby_dcs" {
  ...
  description = <<-EOT
    ...
    (`docs/high-availability.md` §3), and the `standby_join` output prints it. It is not done
...
variable "admin_cidrs" {
  type        = list(string)
  description = <<-EOT
    Who may reach SSH and the web console (web_port). IPv4 and IPv6 prefixes may be
    mixed in one list; each is turned into a rule of its own address family. SSH is open
    to these ranges only; the console is also open to client_cidrs.
...
variable "client_cidrs" {
  ...
  description = <<-EOT
    ...
    MS-XCEP 8446, store 8447, SCEP 8448) and the web console, where users download the
    profiles their devices install. The console requires a login and checks roles, so a
    user can do nothing administrative there. SSH stays with admin_cidrs. Empty means nobody outside the VPC, which is the
