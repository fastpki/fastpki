...
# Every later step applies too, so the database ends at the version the binary requires, not
# at 2: a hard-coded 2 failed the day step 0003 was added.
NEED=$(grep -oE 'kSchemaVersion = [0-9]+' "$ROOT/include/pki/schema.hpp" | grep -oE '[0-9]+')
chk "  the database is now at the binary's version ($NEED)" "$NEED" "$(pg_exec "SELECT max(version) FROM schema_version;")"
