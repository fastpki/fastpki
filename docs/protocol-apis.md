...
| Response 400 | `externalAccountRequired` when the request carries no `externalAccountBinding`, unless a device ticket is outstanding (see *Device attestation* below). An account created without a binding can place device orders and nothing else. |
...
|--------|--------|
| RFC | 8555 §7.4 |
| Request | `{identifiers: [{type: "dns", value: "..."}]}`, or for a device exactly one `{type: "permanent-identifier", value: "<ticket>"}` |
| Response 201 | `{status, expires, identifiers, authorizations, finalize}`, Location: `/acme/order/<id>` |
| Response 400 | `malformed` for any identifier type other than `dns` and `permanent-identifier`, or a device order with more than one identifier; `rejectedIdentifier` when a value is not a syntactically valid DNS name (1–253 bytes, labels 1–63 of letters, digits, `-` and `_`, no empty label, no leading or trailing hyphen, no trailing dot; a leading `*.` is accepted here and the wildcard itself is decided by the profile). Refused at order time rather than at finalize, where the client would already have solved every challenge. |
| Response 403 | `unauthorized` for a device ticket that is unknown, expired, already used or issued for another CA (one answer for all four), when the ticket's owner holds no `acme:enrol` for the CA, and for a `dns` order from an account created without a binding |
...
| Response 400 (device order) | `badCSR` when the CSR's public key is not, byte for byte, the key the device attested. The names come from the CSR and pass the full issuance policy and the ticket's profile, as for SCEP. |
...
| `device-attest-01` | Request `{"attObj": "<base64url CBOR attestation object>"}`, verified in the request: the response carries the challenge with `status: "valid"`, or `400 badAttestationStatement` and the challenge becomes `invalid` |
...
The flow an Apple device runs with an ACME configuration profile and `Attest` on
(draft-ietf-acme-device-attest):

| Step | What happens |
|------|--------------|
| Ticket | An administrator runs `fastpki-acme --issue-device-ticket --ca <ca_id> --owner <user>` ([cli-reference.md](cli-reference.md)). The ticket goes in the profile as `ClientIdentifier`. |
| `new-account` | Accepted without an `externalAccountBinding` while any ticket is unexpired and unclaimed, or while any device serial is registered. |
| `new-order` | One `permanent-identifier`, whose value is either a ticket or a serial number registered for this CA (`/api/device-serials`, the MDM path). A ticket is claimed by the order in one statement, so a second order for it is refused. An order by a registered serial gets a ticket of its own, which expects that serial. Either way the ticket names the CA, the owner RBAC is asked about, and the profile. |
| Authorization | Offers `device-attest-01` only. |
| Challenge | The attestation object must have `fmt` `apple`. Its `x5c` must chain to the Apple Enterprise Attestation Root CA (compiled in) or to a root in `ACME_ATTESTATION_ROOTS`. The leaf's freshness code (`1.2.840.113635.100.8.11.1`) must equal SHA-256 of the challenge token, and the leaf must name a serial number (`1.2.840.113635.100.8.9.1`). For an order by a registered serial, the attested serial must be that one. When `ACME_ATTEST_MIN_OS` is set, the attested OS version (`…8.10.1`) must be at least that, and a missing version is refused; with `ACME_ATTEST_REQUIRE_SIP`, a Mac attesting SIP off (`…8.13.1`) is refused. The serial, UDID and key are recorded. |
| `finalize` | The CSR key must be the attested key. The certificate is issued to the ticket's owner, with the attested serial added as an RFC 4043 permanentIdentifier SAN when its profile sets `device_serial_san`, and its issuance is audited with the device serial. |
| Next certificate | A later certificate for the same attested serial number revokes the previous one with reason 4 (`superseded`). |

Every refusal is written to the service log at `info` with its reason, for example
`ACME POST /acme/<ca_id>/new-account refused 400 externalAccountRequired: …`.
