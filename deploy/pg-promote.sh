...
#   ./pg-promote.sh <service> --already-promoted
#                                       # the database was promoted by db-restore-online.sh:
#                                       # only the steps that follow a promotion
#
...
# ⚠️ --already-promoted: A RESTORE IS A PROMOTION TOO. db-restore-online.sh promotes the standby
# itself, because it holds it read-only while the dump loads, and then hands it here for
# everything a promoted server needs afterwards: the conninfo and anchor for its new role, the
# inherited synchronized_standby_slots, the standby mark, its database certificate, restarted
# listeners. Without these the restored server's own applications kept verifying it against its
# former primary's anchor and could not connect. The mesh check and the promotion are skipped;
# the server must already be a primary.
if [ "$AFTER" = 1 ]; then
  rec="$(pg_query 'SELECT pg_is_in_recovery()')"
  [ "$rec" = f ] || { echo "pg-promote: --already-promoted, but $SVC is not a read-write primary (in recovery: '${rec:-no answer}')" >&2; exit 1; }
else
...
if [ "$rec" = "f" ]; then
  ...
  case "$MODE" in
    ...
    # just promoted down with it. `renew` too: it reads STANDBY_OF when it starts. A protocol
    # switched off (<PROTO>_INSTALLED=false) has no container in the pod, so it is skipped, as
    # native skips a missing init script, rather than reported as a restart that failed.
    k8s)     _have=" $(kubectl get pod -n "$NS" "$SVC" -o 'jsonpath={.spec.containers[*].name}' 2>/dev/null) "
             for _s in ocsp cmp scep est acme ms store web renew; do
                 case "$_have" in *" $_s "*) ;; *) continue ;; esac
  ...
  if [ "$MODE" = k8s ]; then
    ...
    echo
    echo "IMPORTANT: promotion is one-way. To restore redundancy, the OLD primary's pod re-copies"
    echo "its database from this server as a standby — its timeline diverged, and it refuses to"
    echo "start as a second primary. Delete its database claim and pod once its node is back:"
  ...
  echo "      docs/high-availability.md §7 explains why keepalived cannot work in a VPC."
  echo
  echo "IMPORTANT: promotion is one-way. To restore redundancy, re-copy the OLD primary's"
  echo "database from this node, which makes it this node's standby. Never just restart it:"
  echo "its timeline diverged. Its machine, token, keys and certificates stay as they are."
  echo "From your own machine, once the old primary's host is back (docs/high-availability.md §6):"
