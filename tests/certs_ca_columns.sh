...
# only, and there are no migration steps before the release — a deliberate decision — so a
# table added after an install never reaches that deployment. schema_version does not move
# either, so nothing detects it: schema-apply said "nothing to apply", the roll proceeded,
