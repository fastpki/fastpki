...
// ⚠️ CMP'S AUDIT EVENTS ARE WRITTEN FROM OpenSSL'S CALLBACKS, WHICH GET NO REQUEST. The
// issuance, authorization and revocation records below are produced inside the CMP server's
// own processing, several frames down from the HTTP handler, and nothing in that path
// carries the connection. So CMP recorded an empty source address while every other
// protocol recorded one, and an operator reading the audit log saw CMP enrolments that
// appeared to come from nowhere.
//
// The address exists at the HTTP layer — handle_cmp() has the request — so it is stashed
// here for the duration of that request and read at the audit sites. thread_local because
// cpp-httplib serves each connection on its own thread, and a plain global would hand one
// transaction's address to another's audit row under any concurrency at all.
//
// Cleared at the end of every request rather than left to be overwritten: a stale address
// on a record whose request did not set one is worse than an empty field, because it reads
// as evidence.
...
void persist_issued(CmpState& st, X509* cert, const std::string& role,
    ...
    // Audit. The actor is the authenticated owner (CN of the client cert), or empty for a
    // PBM-protected request. The source address comes from g_client_ip, which handle_cmp()
    // sets for the duration of the request — this callback is not request-scoped and cannot
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        pki::log_license(pki::license_status(st.cfg, pki::license_eval_started(*st.db)));
        ...
        auto handle_cmp = [&](const std::string& instance_id,
            ...
            // The one place in CMP that sees the connection. Everything below runs inside
            // OpenSSL's CMP server, which does not, so the address is stashed for this
            // request and cleared however this handler leaves.
            g_client_ip = client_ip(req);
            struct ClearIp {
                ~ClearIp() { g_client_ip.clear(); }
            } clear_ip;
