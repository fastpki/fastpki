...
# ⚠️ EVERY TABLE createdb.sql DECLARES MUST EXIST ONCE THE STEPS HAVE RUN. createdb.sql makes a
# new install and runs only against an empty database; a step carries an existing one forward.
# Releases are public, so every schema change ships as a step, and a table still missing AFTER
# them means a step is missing: a defect in the release, not in the deployment.
#
# Why this is checked at all, measured on an update: five tables added without a step were
# simply absent, schema_version read the same on both versions so nothing detected the gap,
# this script said "nothing to apply", and the rollout then died on
#
...
#
# So: after the steps, compare the tables createdb.sql declares against the ones the database
# has, and refuse BEFORE anything is rolled. Cheap, and it turns a mid-rollout casualty into a
# list of names. ⚠️ AFTER, not before: a step that creates a table is exactly what fills the gap,
# and checking first would refuse every upgrade that adds one.
#
...
check_declared_tables() {
    ...
    echo "" >&2
    echo "      Every step in sql/steps has been applied, and none of them creates these, so" >&2
    echo "      this release is missing a schema step. Please report it." >&2
    echo "" >&2
    ...
    echo "" >&2
    echo "      To carry on meanwhile, create the tables above from sql/createdb.sql on the" >&2
    echo "      PRIMARY — a physical standby receives them automatically — and run this again." >&2
    return 1
}
...
# Now that every step has run, every declared table must exist (see check_declared_tables).
check_declared_tables || exit 1
