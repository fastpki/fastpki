...
4. [Apple macOS and iOS](#4-apple-macos-and-ios)
5. [Constraints inside FastPKI itself](#5-constraints-inside-fastpki-itself)
6. [Testing a relying party before you commit](#6-testing-a-relying-party-before-you-commit)
...
|---|---|---|---|
| **EC** (P-256/384/521) | every stack tested, Windows, macOS and iOS included | the default — smallest certificates, and nothing tested rejects it as a CA key | you must support a client older than roughly 2010 |
| **RSA** (3072+) | every stack tested, Windows, macOS and iOS included | maximum reach, including old appliances | certificate size or signing throughput matters |
| **RSA-PSS** | every stack tested **except macOS and iOS** | a closed estate with no Apple clients | Apple devices are in scope (§4) |
| **Ed25519 / Ed448** | OpenSSL and GnuTLS only — **not Windows, not libpq, not Apple TLS** | a closed estate with no Windows, no Apple clients and no PostgreSQL in the trust path | any of those is in scope (§3, §4, §5.1) |
...
The same limits apply to the key of a **TLS listener** (the console, EST, ACME, MS-XCEP) and
of the **OCSP responder**, because every client has to verify what that key signs. On
macOS and iOS, an `rsa-pss` key and an EC **P-521** key both fail the TLS handshake (§4.2). The
default, EC P-256, works everywhere tested.

Four results from that table:
...
with the PostgreSQL failure and Apple, an Ed25519 CA has three hard holes, not one.

**RSA-PSS is the one RSA form that is not universal.** Windows, OpenSSL, GnuTLS and libpq
accept it; macOS and iOS reject it outright, as a CA key and as a leaf key (§4.1).

**On Windows, the constraint is the CA's key, not the certificate's key.** Windows validated
a chain whose *leaf* carried an Ed25519 or ML-DSA public key without complaint, because an EC
CA had signed it. What it cannot do is verify a signature *made by* one of those keys.
macOS is stricter: for TLS it refuses an Ed25519, Ed448 or ML-DSA key anywhere in the chain,
leaf included (§4.1).
...
only trust anchor. §6.1 has the full method and the client versions.

| CA + leaf key | OpenSSL 3.5.8 (shipped image) | OpenSSL 3.6.3 | Python 3.13 (OpenSSL 3.6) | curl 8.9 (OpenSSL 3.0.16) | GnuTLS 3.8.10 | libpq 16.14 | Windows Server 2022 | macOS 15.5 and 27.0 | iOS 18.5 (Safari) |
|---|---|---|---|---|---|---|---|---|---|
| **RSA-3072** | ok | ok | ok | ok | ok | ok | ok | ok | ok |
| **RSA-PSS** | ok | ok | ok | ok | ok | ok | ok | **fails** | **fails** |
| **EC P-256** | ok | ok | ok | ok | ok | ok | ok | ok | ok |
| **Ed25519** | ok | ok | ok | ok | ok | **fails** | **fails** | **fails** | **fails** |
| **Ed448** | ok | ok | ok | ok | ok | **fails** | **fails** | **fails** | not measured |
| **ML-DSA-44/65/87** | ok | ok | ok | **fails** | **fails** | **fails** | **fails** | **fails** | **fails** (ML-DSA-65) |

macOS and iOS are the only clients here that reject RSA-PSS, and macOS also needs a
`subjectAltName` on every TLS certificate. §4 covers both.

Not tested, and not assumed: Java/JDK, Go, browsers other than Safari on iOS, Android, and
network appliances. Treat every one of them as unknown until measured.
...
number alone is not a safe answer. §6 gives the test.
...
## 4. Apple macOS and iOS

Measured on **macOS 15.5** (build 24F74) and **macOS 27.0** (build 26A428), with the same
results on every case both versions ran. The checks use Apple's Security framework, which
Safari, Mail, Wi-Fi and every app that uses Apple's networking rely on. macOS does not use
OpenSSL for this. `/usr/bin/curl` on a Mac is built on LibreSSL, so it does **not** show what
Safari or the system will accept.

Two checks were made per case, each with the test CA as the only trust anchor. The table
below is the macOS 15.5 run; the P-384 and P-521 rows were measured on 15.5 only.

- **path** — `SecTrustEvaluateWithError` with the TLS server policy (`SecPolicyCreateSSL`):
  can macOS build and accept the chain for a server of that name?
- **TLS** — an HTTPS request through `NSURLSession` to the leaf, served by
  `openssl s_server -tls1_3` from the shipped image: does the handshake complete?

| CA key | leaf key | path | TLS handshake |
|---|---|---|---|
| RSA-3072 | RSA-3072 | ok | ok |
| EC P-256 | EC P-256 | ok | ok |
| EC P-384 | EC P-384 | ok | ok |
| EC P-521 | EC P-521 | ok | **fails** |
| RSA-PSS | RSA-PSS | **fails** — "using a broken key size" | **fails** |
| RSA-PSS | EC P-256 | **fails** — "not trusted" | **fails** |
| Ed25519 or Ed448 | same | **fails** — "using a broken key size" | **fails** |
| Ed25519 or Ed448 | EC P-256 | **fails** — "using a broken key size" | **fails** |
| EC P-256 | Ed25519 | **fails** — "using a broken key size" | **fails** |
| ML-DSA-44/65/87 | same | **fails** — "using a broken key size" | **fails** |
| ML-DSA-65 | RSA-3072 | **fails** — "not trusted" | **fails** |
| EC P-256 | ML-DSA-65 | **fails** — "using a broken key size" | **fails** |

EC and plain RSA are safe for any Apple client. Every other CA key fails.

### 4.1 Why each one fails

**RSA-PSS.** macOS cannot verify an RSASSA-PSS signature on a certificate. This holds for an
RSA-PSS key and for an ordinary RSA key signing with PSS (SHA-256, MGF1-SHA-256, 32-byte
salt, the standard parameters). It also refuses a public key of type RSASSA-PSS in any
certificate. FastPKI's RSA-PSS CA does both, so a Mac rejects everything below it.

**Ed25519 and Ed448.** macOS *can* verify these signatures. With the basic X.509 policy,
`security verify-cert -p basic` accepts an Ed25519 chain, and rejects the same chain with
one byte of the signature changed. What fails is the TLS policy: it treats any Ed25519 or
Ed448 key in the chain, leaf or CA, as a key of size 0 and refuses it as too weak. So an
EdDSA CA is unusable for HTTPS, and the error message points at key size, not at the
algorithm.

**ML-DSA.** Neither version recognises the algorithm. The basic policy reports
`CSSMERR_TP_INVALID_CERTIFICATE`, the TLS policy reports "using a broken key size", and an
RSA leaf signed by an ML-DSA CA is simply "not trusted".

### 4.2 Keys for TLS listeners and the OCSP responder

A P-521 **CA** is fine: macOS verifies the chain. A P-521 **server key** is not: Apple's TLS
stack does not offer the P-521 signature scheme, so the handshake ends before any
certificate is checked:

```
NSURLErrorDomain -1200: An SSL error has occurred and a secure connection to the server cannot be made.
```

The same error appears for an `rsa-pss`, Ed25519, Ed448 or ML-DSA server key. If Macs will
connect, give the console, EST, ACME and MS-XCEP listeners an `ec` key on P-256 or P-384, or
an `rsa` key. The OCSP responder signs responses rather than a handshake, so P-521 is fine
there, but `rsa-pss` is not: macOS cannot verify its signatures (§4.1).

### 4.3 The error messages mislead

- **"certificate is using a broken key size"** (`CSSMERR_CSP_UNSUPPORTED_KEY_SIZE` from
  `security verify-cert`) means the key's algorithm is not accepted for this use. It does not
  mean the key is too short.
- **"certificate is not trusted"** on a chain whose anchor *is* trusted means macOS could not
  verify a signature in it — RSA-PSS or ML-DSA.
- **-1200** from Safari or any app means the handshake failed, often because of the server's
  key type. Test the chain with `security verify-cert` (§6) to tell the two apart.

### 4.4 A TLS certificate needs a subjectAltName and serverAuth

macOS ignores the Common Name. A certificate without a `subjectAltName` fails for TLS
whatever its key algorithm, even though its chain verifies:

| certificate | macOS verdict |
|---|---|
| CN only, no SAN, no EKU | `Invalid Extended Key Usage for policy`, then `Host name mismatch` |
| CN, `SAN: DNS:<name>` and `serverAuth` EKU | trusted, and the TLS handshake completes |

FastPKI issues both extensions when the CSR asks for them, so the CSR has to ask. The error
names the host name, not the missing SAN.

### 4.5 iOS

Measured on **iOS 18.5** with Safari. Each test root was installed from a configuration
profile and given full trust under **Settings → General → About → Certificate Trust
Settings**. Then the page served with each root's leaf certificate was opened:
...
These are the same verdicts macOS gives, P-521 included. Every root profile installed without
error, including the ones whose chains then fail. So, as on Windows, a root that installs
cleanly is no evidence that iOS can verify what it signs.

A root installed from a hand-installed profile is not trusted for TLS until full trust is
turned on in **Certificate Trust Settings**. This is the iOS form of the macOS behaviour in
the user guide ([user-guide.md](user-guide.md) §8.5).

### 4.6 Other Apple versions

Apple adds post-quantum support in newer releases, so the ML-DSA rows in particular may
change. Re-measure on the versions your devices run before relying on a "fails" cell.

---

## 5. Constraints inside FastPKI itself
...
### 5.1 PostgreSQL certificates must come from an EC or RSA CA
...
it, as §6.1 says of every cell.

### 5.2 A SCEP RA key must be RSA
...
### 5.4 Full-chain CRL checking needs the CRLs staged, not downloaded
...
### 5.5 Creating the key needs the shipped image
...
## 6. Testing a relying party before you commit
...
`could not find digest for NID UNDEF` is §5.1. Re-run it with `channel_binding=disable`
...
**macOS**, with the Mac's own `security` tool. It uses Apple's trust code, and it needs
neither a keychain change nor administrator rights:

```bash
security verify-cert -c leaf.crt -c sub_ca.crt -r root.crt -p ssl -s <leaf-dns-name> -L
```

`...certificate verification successful.` means Safari and the system will accept the chain.
`CSSMERR_CSP_UNSUPPORTED_KEY_SIZE` means an algorithm in the chain is refused for TLS, and
`CSSMERR_TP_NOT_TRUSTED` or `CSSMERR_TP_INVALID_CERTIFICATE` means macOS could not verify a
signature in it (§4.3). The `Certificate Transparency` lines printed after the result apply
to public CAs only; ignore them. Replace `-p ssl -s <leaf-dns-name>` with `-p basic` to check
the signatures alone, without the TLS rules. Do not use `/usr/bin/curl` for this: it is
built on LibreSSL, not on Apple's trust code.
...
### 6.1 How the table in §2 was produced
...
The macOS column was measured twice, on two Macs, with the test CA as the only anchor each
time:

- **macOS 15.5** (24F74). The certificates were generated with OpenSSL 3.5.8 inside the
  shipped image and served from it with `openssl s_server -tls1_3`. The Mac ran
  `SecTrustEvaluateWithError` with `SecPolicyCreateSSL`, and an HTTPS request through
  `NSURLSession`. `security verify-cert` gave the same verdicts. A copy of an Ed25519 leaf with
  one signature byte changed confirmed that the basic policy really checks EdDSA signatures
  (§4.1).
- **macOS 27.0** (26A428) on arm64. Each chain was checked with
  `security verify-cert -p ssl -s localhost`, and each leaf was fetched by a `URLSession`
  client that pinned the CA with `SecTrustSetAnchorCertificatesOnly`. OpenSSL 3.6.3 on the
  same Mac accepted all eight chains, so the failures come from Apple's checks, not from the
  certificates.

Both runs separated the CA key from the leaf key, as the Windows column did. The §4.4 results
come from certificates FastPKI issued over EST, checked on macOS 27.0.

The iOS column used an iPhone on iOS 18.5. The same certificates, generated and served from
the shipped image, were opened in Safari after each test root was installed from a
configuration profile and given full trust. Ed448 and ML-DSA-44/87 were not tried there.
...
libpq 16.14, Windows Server 2022 Standard, macOS 15.5 (24F74) and 27.0 (26A428), and iOS 18.5. **Re-measure before relying on a "fails" cell** — these move with client
