...
namespace {
constexpr int kHttp01MaxRedirects = 10;
struct Http01Refused { const char* cidr; const char* what; };
constexpr Http01Refused kHttp01Refused[] = {
    {"0.0.0.0/8",      "an unspecified address"},
    {"169.254.0.0/16", "a link-local address (this includes cloud metadata endpoints)"},
...
// One http-01 URL, split. `host` is bare (no brackets around an IPv6 literal).
struct Http01Url { std::string scheme, host, path; int port = 0; };

// Parses an absolute http(s) URL, or resolves `loc` against `base` when it is relative
// ("/path" or "//host/path"). Empty `why` on success.
Http01Url http01_parse(const std::string& loc, const Http01Url* base, std::string& why) {
    Http01Url u;
    std::string rest;
    if (base && loc.rfind("//", 0) == 0) { u.scheme = base->scheme; rest = loc.substr(2); }
    else if (base && !loc.empty() && loc[0] == '/') {
        u = *base; u.path = loc; return u;
    } else {
        const auto sep = loc.find("://");
        if (sep == std::string::npos) { why = "a redirect to '" + loc + "', which is not an http or https URL"; return u; }
        u.scheme = loc.substr(0, sep);
        std::transform(u.scheme.begin(), u.scheme.end(), u.scheme.begin(),
                       [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
        rest = loc.substr(sep + 3);
    }
    if (u.scheme != "http" && u.scheme != "https") {
        why = "a redirect to a '" + u.scheme + "' URL; only http and https are followed";
        return u;
    }
    ...
    if (authority.find('@') != std::string::npos) { why = "a redirect whose URL carries credentials"; return u; }
    std::string port;
    if (!authority.empty() && authority[0] == '[') {
        const auto close = authority.find(']');
        if (close == std::string::npos) { why = "a redirect to a malformed address"; return u; }
    ...
    if (u.host.empty()) { why = "a redirect with no host"; return u; }
    if (port.empty()) u.port = (u.scheme == "https") ? 443 : 80;
    else if (port.find_first_not_of("0123456789") != std::string::npos || port.size() > 5) {
        why = "a redirect to a malformed port"; return u;
    } else u.port = std::stoi(port);
    if (u.port != 80 && u.port != 443) {
        why = "a redirect to port " + std::to_string(u.port) + "; only ports 80 and 443 are followed";
        return u;
    }
    return u;
}

// The addresses to dial for `host`, in resolver order, refused ones dropped. Empty, with `why`
// set, when it does not resolve or every address it has is refused. All of them, not the
// first: a name with a dead IPv6 route still answers on IPv4, and the HTTP client this
// replaced tried each in turn.
std::vector<std::string> http01_addresses(const std::string& host, int port, std::string& why) {
    struct addrinfo hints{}, *ai = nullptr;
    hints.ai_family = AF_UNSPEC; hints.ai_socktype = SOCK_STREAM;
    if (getaddrinfo(host.c_str(), std::to_string(port).c_str(), &hints, &ai) != 0 || !ai) {
        why = "'" + host + "' does not resolve";
        return {};
    }
    std::unique_ptr<struct addrinfo, decltype(&freeaddrinfo)> guard(ai, &freeaddrinfo);
    std::vector<std::string> out;
    std::string refused;
    for (auto* p = ai; p; p = p->ai_next) {
        char buf[INET6_ADDRSTRLEN] = {0};
        const void* src = p->ai_family == AF_INET
            ? static_cast<const void*>(&reinterpret_cast<sockaddr_in*>(p->ai_addr)->sin_addr)
            : static_cast<const void*>(&reinterpret_cast<sockaddr_in6*>(p->ai_addr)->sin6_addr);
        if (!inet_ntop(p->ai_family, src, buf, sizeof buf)) continue;
        const std::string ip = buf;
        const Http01Refused* hit = nullptr;
        for (const auto& r : kHttp01Refused) if (pki::ip_in_cidr(ip, r.cidr)) { hit = &r; break; }
        if (!hit) {
            if (std::find(out.begin(), out.end(), ip) == out.end()) out.push_back(ip);
            continue;
        }
        if (refused.empty()) refused = "'" + host + "' resolves to " + ip + ", " + hit->what +
                                       ", which is never a validation target";
    }
    if (out.empty())
        why = refused.empty() ? "'" + host + "' resolves to no usable address" : refused;
    return out;
}
} // namespace
...
// ⚠️ REDIRECTS ARE FOLLOWED BY HAND, AND EVERY HOP IS CHECKED BEFORE IT IS DIALLED. This used
// httplib's set_follow_location(true), which follows any redirect to any host, port and
// scheme: a challenge response could send the CA to an internal service on any port, or to
// the link-local metadata endpoint (169.254.169.254) of the cloud it runs in. Blind, because
// validation still needs the final body to be the key authorization, but a request the CA
// should never make.
//
// RFC 8555 §8.3 says the server SHOULD follow redirects (http -> https is the everyday case),
// so they are followed, within these rules:
//   - at most kHttp01MaxRedirects of them;
//   - http or https only, on port 80 or 443;
//   - every hop's address is checked as the first one is, and the connection is PINNED to the
//     address that was checked, so a name cannot resolve to one address for the check and
//     another for the connection.
// Private ranges and loopback are allowed: a private CA validates names that resolve to
// private addresses, and a single-host install validates over loopback. Refused are the
// addresses no validation target can legitimately have (see kHttp01Refused).
bool fetch_http01(const std::string& host, const std::string& token,
                  const std::string& expected_key_auth, std::string* why) {
    auto fail = [&](const std::string& reason) { if (why) *why = reason; return false; };
    std::string err;
    Http01Url url{"http", host, std::string("/.well-known/acme-challenge/") + token, 80};
    for (int hop = 0;; ++hop) {
        // Resolved and checked here rather than left to the HTTP client, because "cannot
        // resolve", "refused address" and "connected and answered wrongly" are different
        // operator problems, and httplib reports all of them as an empty result.
        const auto addrs = http01_addresses(url.host, url.port, err);
        if (addrs.empty())
            return fail(hop == 0 && err.find("does not resolve") != std::string::npos
                        ? std::string("the identifier does not resolve")
                        : (hop == 0 ? err : "the challenge URL redirected to " + err));
        const bool v6 = url.host.find(':') != std::string::npos;
        httplib::Result r;
        for (const auto& addr : addrs) {
            httplib::Client cli(url.scheme + "://" + (v6 ? "[" + url.host + "]" : url.host) +
                                ":" + std::to_string(url.port));
            // Pinned: the connection goes to the address checked above, never to whatever
            // the name resolves to a moment later.
            cli.set_hostname_addr_map({{url.host, addr}});
            cli.set_connection_timeout(5);
            cli.set_read_timeout(10);
            cli.set_follow_location(false);
            // An https hop is not a certificate check: the key authorization is what proves
            // control, and RFC 8555 §8.3 asks for nothing more.
            cli.enable_server_certificate_verification(false);
            r = cli.Get(url.path);
            if (r) break;
        }
        if (!r)
            return fail(hop == 0 ? std::string("nothing answered on port 80 at the identifier")
                                 : "nothing answered at " + url.scheme + "://" + url.host + ":" +
                                   std::to_string(url.port) + " after a redirect");
        const int st = r->status;
        if (st == 301 || st == 302 || st == 303 || st == 307 || st == 308) {
            if (hop >= kHttp01MaxRedirects)
                return fail("the challenge URL redirected more than " +
                            std::to_string(kHttp01MaxRedirects) + " times");
            const std::string loc = r->get_header_value("Location");
            if (loc.empty()) return fail("a redirect with no Location");
            err.clear();
            Http01Url next = http01_parse(loc, &url, err);
            if (!err.empty()) return fail("the challenge URL answered with " + err);
            url = next;
            continue;
        }
        if (st != 200)
            return fail("the challenge URL answered HTTP " + std::to_string(st) + ", not 200");
