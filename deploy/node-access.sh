...
#   PORTS     the port a peer dials at each of those addresses, comma-separated in the same
#             order; empty means 5432 at every address (native and Compose publish 5432)
...
na_facts() {   # na_facts <server>
    ...
    case "$NA_KIND" in
        ...
        k8s)
            ...
            echo "BIND=$_bind"
            # Each server's database is published on its own port (apply.sh 7b), read back from
            # the Service rather than assumed: its node port for NodePort, else its port.
            _ports="" _k=0
            for _a in $(printf '%s' "$_bind" | tr ',' ' '); do
                _pp=$(na_kubectl get service "postgres-external-$_k" \
                        -o 'jsonpath={.spec.type} {.spec.ports[0].port} {.spec.ports[0].nodePort}' 2>/dev/null \
                      | awk '{ print ($1 == "NodePort") ? $3 : $2 }')
                _ports="$_ports${_ports:+,}$_pp"
                _k=$((_k + 1))
            done
            echo "PORTS=$_ports"
