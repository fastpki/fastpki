...
> **📷 Screenshot** — `images/console-login.png`
> The login page: username and password, the domain list if the deployment has
> directories, and a **Sign in with …** button for each single-sign-on provider.
...
> **📷 Screenshot** — `images/inventory-request-buttons.png`
> Console → Inventory, top of the page: the request buttons, with a couple of
> already-issued certificates listed below them.
...
> **📷 Screenshot** — `images/request-key-in-browser.png`
> The request dialog: Issue from, the subject fields, the SAN box, key algorithm and size,
> key usage and extended key usage, and the Output section with the password and the
> download format.
...
3. Add the additional names (**SANs**), one per line. **Nothing is added for you**: for a
   server certificate, put its DNS name here, even if it is already the CN.
4. Pick the key: RSA, RSA-PSS, EC or Ed25519, with a size or curve.
...
> **📷 Screenshot** — `images/inventory-list.png`
> The inventory table: CN, serial, issuing CA, owner, status and expiry. Include one revoked
> row so the status styling shows.
...
| Typical client | certbot, acme.sh, Caddy, Traefik | `openssl` + curl, Cisco/Aruba gear | MDM platforms, routers, firewalls | `openssl cmp` | Windows itself |
...
> **📷 Screenshot** — `images/enrolment-credentials.png`
> Dashboard → My enrolment credentials, with the values hidden. Show the Reveal button, the
> four download buttons and the Files enrol against list.

Below the values are four download buttons:
...
deployment's address for that CA, and the CMP, ACME and SCEP files also carry your
...
answer on port 80/443 or write a DNS record.
...
routers, switches, firewalls, VPN concentrators, and every MDM platform. A CMS-wrapped
...
| macOS | Keychain Access → System → drag in, then set "Always Trust" |
