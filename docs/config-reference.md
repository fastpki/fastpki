...
![The console's Config page, Identity area](images/admin-config-identity.png)
...
| `LOGIN_FAILURE_THRESHOLD` | int | `5` | Consecutive failed sign-ins tolerated, per account and per client address, before a delay applies. Below this nothing happens. Behind a proxy, set `TRUSTED_PROXIES` or every client counts as one address and the delay becomes deployment-wide. |
...
![The console's Config page, Certificate Authority area](images/admin-config-ca.png)
...
![The console's Config page, Key Management area](images/admin-config-hsm.png)
...
![The console's Config page, Database area](images/admin-config-db.png)
...
![The console's Config page, Datacenter area](images/admin-config-dc.png)
...
![The console's Config page, OCSP & CRL area](images/admin-config-ocsp-crl.png)
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `WEB_KEY_ALGO` | str | `ec` | Algorithm of the key the **web console TLS** generates for itself in the token at first start: `ec`, `rsa` or `rsa-pss`. A TLS listener key is limited to these three because a peer has to negotiate it (see `docs/compatibility.md` §1). Apple clients refuse an `rsa-pss` or P-521 listener key (§4.2 there). |
...
| `EST_KEY_ALGO` | str | `ec` | Algorithm of the key the **EST listener TLS** generates for itself in the token at first start: `ec`, `rsa` or `rsa-pss`. A TLS listener key is limited to these three because a peer has to negotiate it (see `docs/compatibility.md` §1). Apple clients refuse an `rsa-pss` or P-521 listener key (§4.2 there). |
...
| `ACME_KEY_ALGO` | str | `ec` | Algorithm of the key the **ACME listener TLS** generates for itself in the token at first start: `ec`, `rsa` or `rsa-pss`. A TLS listener key is limited to these three because a peer has to negotiate it (see `docs/compatibility.md` §1). Apple clients refuse an `rsa-pss` or P-521 listener key (§4.2 there). |
...
| `MS_KEY_ALGO` | str | `ec` | Algorithm of the key the **MS-XCEP/WSTEP listener TLS** generates for itself in the token at first start: `ec`, `rsa` or `rsa-pss`. A TLS listener key is limited to these three because a peer has to negotiate it (see `docs/compatibility.md` §1). Apple clients refuse an `rsa-pss` or P-521 listener key (§4.2 there). |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `OCSP_RESPONDER_KEY_ALGO` | str | `ec` | Algorithm of the **OCSP responder** key: `ec`, `rsa` or `rsa-pss`. A responder answers every client that asks, including Windows and libpq, so it is held to the same reach as a listener. Apple clients cannot verify an `rsa-pss` responder (`docs/compatibility.md` §4.2). |
...
![The console's Config page, EST area](images/admin-config-est.png)
...
![The console's Config page, ACME area](images/admin-config-acme.png)
...
| `ACME_DNS_RESOLVER` | string | *(empty)* | DNS-01 + CAA validator resolver, `host[:port]` — an IPv4 or IPv6 literal or a **name** (bracket a v6 literal to give it a port: `[::1]:5353`). Resolved and queried from inside `fastpki-acme`, so under compose or Kubernetes it must name something reachable on that container's own network; a resolver listening only on the docker host is not, and the unanswered query is reported as `dns-01 TXT record missing or mismatched`. Empty = `/etc/resolv.conf`. **On an HA pair, give an address both hosts can reach.** This setting is one row in the shared `config` table, so both hosts read the same value, but each has its own container network — a container name that resolves on one host resolves nowhere on the other. Either host may answer a validation, so half of them fail, and the failure names the TXT record rather than the resolver. |
...
| `ACME_ATTESTATION_ROOTS` | string | *(empty)* | Trust anchors for ACME device attestation (`device-attest-01`) **in addition to** the Apple Enterprise Attestation Root, which is compiled in and always trusted: PEM certificates, or the same PEM base64-encoded on one line. It adds attestors and cannot remove Apple's root or skip any check. Read at startup. |
| `ACME_ATTEST_MIN_OS` | string | *(empty)* | Device attestation: refuse an Apple device whose attested OS version is lower, compared numerically (`26.0`). A device that attests no version, one older than iOS 17.2 or macOS 14.2, is refused while this is set. macOS and iOS share version numbers from 26, so a minimum of `26.0` also turns away macOS 15 and iOS 18. Empty = no check. Read at startup. |
| `ACME_ATTEST_REQUIRE_SIP` | bool | `false` | Device attestation: refuse a Mac whose attestation says System Integrity Protection is off. An attestation with no SIP value, from an iPhone, an iPad or a Mac too old to report it, passes; set `ACME_ATTEST_MIN_OS` as well to turn away the old Macs. Read at startup. |
| `ACME_SWEEP_SEC` | int | `600` | Background sweep for expired nonces/orders. |

## 10. Notify (Expiry Notifications)

![The console's Config page, Notifications area](images/admin-config-notifications.png)
...
![The console's Config page, Updates area](images/admin-config-updates.png)
...
FastPKI is free for non-commercial use under PolyForm Noncommercial 1.0.0. A commercial
deployment needs a licence from us. These two keys record the licence that was issued, so the
console and the logs can state which it is.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `LICENSE` | string | *(empty)* | The licence file's text. This is what the console's upload writes. |
| `LICENSE_FILE` | path | *(empty)* | A path to the licence file instead, for a deployment that would rather mount one. `LICENSE` wins when both are set. |

**The easiest way is the Version page**: choose the licence file we sent you, or paste it, and
press **Install licence**. The console checks the signature before storing it, so a file that
was pasted wrong is refused there and then rather than leaving the deployment in a "not valid"
state you have to go and diagnose. **Remove** puts the deployment back on the evaluation.

The text is stored in the `config` table, not written to a file — so there is nothing to copy
onto the server, which matters most on Compose and Kubernetes where putting a file on the node
means a volume mount. On a mesh it reaches every node by replication, so it is uploaded once.

From a script: `fastpki-config set LICENSE "$(cat acme.license)"`. Both keys have to go in the
database rather than `bootstrap.conf`, because the licence is several lines and that file is
read one line at a time.

`LICENSE_FILE` exists for a deployment that would rather mount the file — a Kubernetes Secret,
say. Use one or the other.

**Nothing enforces any of this.** FastPKI does not refuse to start, limit what it will issue,
turn a feature off, stop working after the expiry date, or contact us. Every field is
displayed and logged, and acted on nowhere. It exists so an operator, an auditor or a buyer
can see the deployment's licence state without asking us — an unlicensed commercial
deployment otherwise looks exactly like a licensed one to everybody, including the people
running it.

### The evaluation

Every build carries a 30-day evaluation licence, compiled in and identical for everyone. It
applies whenever `LICENSE_FILE` is empty, so a new install is in evaluation rather than being
told it is unlicensed on day one.

It carries a *period*, not a date — a date fixed when the release was built would be spent
before most people downloaded it. The node records the day it first ran in the `config` table
and counts thirty days from there. That row is bookkeeping the node writes for itself, not a
setting to change. Dropping the database or
reinstalling starts the evaluation again; that is accepted, because the contract is the
instrument that matters and this is a courtesy.
...
MEUCIQD37OkfZuleaxO3cU2W4Ux5ovWD3NNoofOxWBgHgBOxLwIgEr5Ajoz9Zih3
xJD9tGBLAB/XYeDMIO5XWgHgd7ISl0Y=
-----END FASTPKI LICENSE SIGNATURE-----
```

Omit `expires` for a perpetual licence. `tier` and `nodes` are reported and never checked
against anything.

The signature is made with the FastPKI licence key and covers the bytes between the first pair
of markers, exactly as written, so you can check it yourself:

```bash
sed -n '/BEGIN FASTPKI LICENSE-/,/END FASTPKI LICENSE-/p' acme.license | sed '1d;$d' > body
sed -n '/BEGIN FASTPKI LICENSE SIGNATURE/,/END FASTPKI LICENSE SIGNATURE/p' acme.license \
    | sed '1d;$d' | base64 -d > sig
openssl dgst -sha256 -verify license-pub.pem -signature sig body
```

The public key is compiled into the programs rather than read from a path: pinning that an
operator has to configure is not pinning, and a key on disk is a key that can be swapped.

A file that does not parse or whose signature does not verify is reported as **not valid** —
FastPKI keeps running, and does not quietly fall back to the evaluation, because that would
hide a broken licence behind a state that looks fine for thirty days.

### Where the state appears

- The **Version** page: the number, who it was issued to, the tier, the node count and the
  expiry — or the evaluation's end date, or why a file was rejected.
- Every service writes one line to its log at startup. An ended evaluation, an expired licence
  and an invalid file go to `err` so they are visible at the default log level; a licence in
  good standing and a running evaluation go to `info`, so the log still answers "which licence
  was this node running under?".
- The console shows a marker beside the navigation on every page unless a licence is in force.
  Somebody who has paid never sees it.

## 12. Discovery

![The console's Config page, Discovery area](images/admin-config-discovery.png)
...
![The console's Config page, CMP area](images/admin-config-cmp.png)
...
![The console's Config page, MS-XCEP/WSTEP area](images/admin-config-ms.png)
...
![The console's Config page, Certificate Store area](images/admin-config-store.png)
...
![The console's Config page, Web Console area](images/admin-config-web-console.png)
...
![The console's Config page, SCEP area](images/admin-config-scep.png)
...
![The console's Config page, Issuance Policy area](images/admin-config-policy.png)
...
![The console's Config page, Authentication area](images/admin-config-auth.png)
...
![The console's Config page, Logging area](images/admin-config-logging.png)

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `LOG_LEVEL` | string | `err` | `err` / `info` / `debug`. |
| `TRUSTED_PROXIES` | csv | *(empty)* | Addresses or CIDR prefixes of proxies whose `X-Forwarded-For` is believed. Empty means the header is ignored and the address on the connection is recorded. Pending restart, on every listener. |

### Recording the real client behind a proxy

Put a load balancer or a reverse proxy in front of FastPKI and every request arrives from
that proxy's address. Two things then stop working:

- The audit log stamps every issuance, revocation and configuration change with the same
  address, so it cannot say who did anything.
- `LOGIN_FAILURE_THRESHOLD` counts failures per account **and** per client address. With one
  address for everybody, the per-address half becomes a deployment-wide delay, so one
  person's mistyped password slows down everyone else.

`TRUSTED_PROXIES` names the proxies to believe. Each entry is an address or a CIDR prefix,
and both address families work:

```
TRUSTED_PROXIES=10.20.0.0/24, 2001:db8:1::5
```

With it set, a request arriving **from one of those addresses** has its `X-Forwarded-For`
read, and the client's own address is recorded. A request from anywhere else is recorded by
its connection address whatever headers it sends.

⚠️ **Name only proxies you run.** `X-Forwarded-For` is written by whoever sends it. Believing
it from an address you do not control lets that caller choose what the audit log says about
them, and lets them slip the sign-in backoff by inventing a new address for every attempt —
worse than recording the proxy honestly. That is why there is no default and why an empty
value ignores the header entirely.

The header is read right to left. Each proxy appends the address it saw, so the entries
nearest the end are the ones your own proxies wrote; the first entry from the right that is
not itself a trusted proxy is the client. A caller can put anything it likes at the front of
that list before your proxy ever sees it, which is why the leftmost entry is not used.

An IPv4 client reaching a dual-stack listener arrives as an IPv4-mapped address
(`::ffff:10.20.0.7`). Write your prefixes in the ordinary IPv4 form — `10.20.0.0/24` matches
that client.
...
Most keys can be set via an environment variable of the same name — 127 of the 162 the
...
- `LICENSE`, `LICENSE_FILE`, `OCSP_RESPONDER_KEYS_REPLICATED`
...
⚠️ This list and the one in `deployment.md` are the same set: the keys `apply()` parses
minus the ones `from_env()` reads. Both are maintained by hand, and `tests/config_keys_live.sh`
fails when either list, or a count on this page or in `deployment.md`, no longer matches the code.
