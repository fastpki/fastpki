...
#include "pki/log.hpp"
#include "pki/pkcs11_helpers.hpp"  // pkcs11_uri_redacted
...
EvpPkeyPtr load_ca_key_or_explain(const std::string& ref, const Config& cfg,
    ...
    // ⚠️ REDACTED: RFC 7512 lets the PIN sit in the URI as pin-value=, and this sentence is
    // logged — on a mesh, once per peer CA on every start of every node.
    const std::string shown = pkcs11_uri_redacted(ref);
    try {
        auto k = load_signing_key(ref, cfg);
        if (k) return k;
        why = "CA '" + ca_id + "': " + shown + " resolved to no key.";
    } catch (const std::exception& e) {
        why = "CA '" + ca_id + "': its signing key " + shown + " is not available on this "
...
X509Ptr Responder::resolve_responder_cert(std::string& why, bool& elsewhere) const {
    elsewhere = false;
    if (!responder_key_) {
    ...
    if (!pki::cert_certifies_key(cert.get(), responder_key_.get())) {
        // ⚠️ ON A MESH THIS IS USUALLY NOT A FAULT. Every data center holds every CA's rows,
        // including the responder certificate its home data center issued for ITS OWN
        // responder key. Asked about a peer's CA, this node found that certificate, saw it
        // certify some other key, and answered internalError with an ERR line telling the
        // operator to reissue it — which, done here, would replace the home data center's
        // credential mesh-wide. When this node cannot load the CA's key either, neither
        // credential is here: the CA is answered elsewhere, and handle() says so.
        elsewhere = !ca_key_;
...
std::vector<unsigned char> Responder::handle(const unsigned char* req_der, size_t len) {
    try {
        ...
        X509Ptr responder_cert = responder_key_ ? resolve_responder_cert(why, elsewhere) : nullptr;
        // RFC 6960 §2.3: `unauthorized` is "this responder is not the one for that
        // certificate", which is the truth here; internalError says the server is broken.
        if (!responder_cert && elsewhere) {
            if (!elsewhere_logged_.exchange(true))
                log::info("OCSP: CA '" + ca_id_ + "' is answered by another node: this node "
                          "holds neither its signing key nor the key its responder "
                          "certificate '" + cfg_.ocsp_responder_cert_id_prefix + "-" + ca_id_ +
                          "' certifies. Requests for it here are answered `unauthorized`.");
            return status_response(OCSP_RESPONSE_STATUS_UNAUTHORIZED);
        }
        if (!responder_cert) {
            if (!responder_key_)
