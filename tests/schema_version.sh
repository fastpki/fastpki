...
#
# ⚠️ THAT SCENARIO IS NOW UNREACHABLE, and the assertion follows it. The 39 steps were
# collapsed into sql/createdb.sql — they only ever ran against an already-deployed database
# and there are none — so the only way a database now comes into existence is createdb.sql,
# which CREATES schema_version and seeds it in the same file. There is no longer a moment
# at which a database has the schema and cannot say so. Asserted on the file rather than by
# dropping the table and hoping a step puts it back, which is what used to heal it.
...
# When steps DO come back, they must still carry an older database to the required version.
