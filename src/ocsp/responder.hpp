...
private:
    ...
    // operator can act on. `elsewhere` is set when the refusal means "this CA is answered
    // by another node", not "this node is misconfigured" — see handle().
    X509Ptr resolve_responder_cert(std::string& why, bool& elsewhere) const;
    ...
    // A CA answered by another node is reported once per responder, not once per request:
    // anyone can ask about it, and a line per request is a log anyone can fill.
    std::atomic<bool> elsewhere_logged_{false};
};
