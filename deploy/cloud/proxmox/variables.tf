...
variable "template_vm_id" {
  type        = number
  description = <<-EOT
    VMID of the FastPKI template to clone. This module does NOT build it — the image is
    baked by deploy/cloud/image/build-qemu.sh and turned into a template once, the same
    separation the AWS module has between packer and an AMI id.

    The template must be the `-uefi-` cloud image with bios=ovmf and an EFI disk, because
    that is what the qcow2 is: GPT with an EFI system partition and no BIOS boot code.
  EOT
}
...
variable "key_backend" {
  type        = string
  default     = "softhsm"
  description = "Where CA private keys live. softhsm is the lab default; a real deployment points this at an HSM through PKCS#11."
  validation {
    condition     = contains(["softhsm", "pkcs11"], var.key_backend)
    error_message = "key_backend must be softhsm or pkcs11."
  }
}
