...
# ⚠️ Every run here is `sh "$SH"`, never `bash "$SH"`: the guide says `./install.sh`, whose
# shebang is /bin/sh — dash on Debian, busybox ash here. Under bash this suite passed while
# the wizard died at its first question on Debian ("printf: Illegal option -v"), and busybox
# silently left every answer empty.
gen(){ printf '%s\n' "$1" > ans.env; sh "$SH" --answers ans.env --print-env 2>/dev/null; }
...
sh "$SH" --answers toobig.env --print-env >/dev/null 2>&1
chk "DC_INDEX above 32767 -> non-zero exit" fail "$([ $? -ne 0 ] && echo fail || echo ok)"
printf 'DEPLOYMENT=cluster\nDC_INDEX=32767\n' > atmax.env
chk "  ... and 32767 is accepted" 32767 \
    "$(field "$(sh "$SH" --answers atmax.env --print-env 2>/dev/null)" DATACENTER_ID)"
...
chk "a fourth data center installs without touching the others" 4 \
    "$(field "$(sh "$SH" --answers ok4.env --print-env 2>/dev/null)" DATACENTER_ID)"
printf 'DEPLOYMENT=cluster\nDC_INDEX=0\n' > bad.env
sh "$SH" --answers bad.env --print-env >/dev/null 2>&1; chk "DC_INDEX 0 -> non-zero exit" fail "$([ $? -ne 0 ] && echo fail || echo ok)"
printf 'DEPLOYMENT=weird\n' > bad2.env
sh "$SH" --answers bad2.env --print-env >/dev/null 2>&1; chk "bad DEPLOYMENT -> non-zero exit" fail "$([ $? -ne 0 ] && echo fail || echo ok)"
printf 'DEPLOYMENT=cluster\nDC_INDEX=abc\n' > bad3.env
sh "$SH" --answers bad3.env --print-env >/dev/null 2>&1; chk "non-numeric DC_INDEX -> non-zero exit" fail "$([ $? -ne 0 ] && echo fail || echo ok)"
...
BARE=$(mktemp -d)
for t in sh sed grep printf cut head tr; do
    p=$(command -v "$t" 2>/dev/null)
    case "$p" in /*) ln -sf "$p" "$BARE/$t" 2>/dev/null ;; esac
done
printf 'DEPLOYMENT=cluster\nDC_INDEX=2\n' > np.env
NP=$(PATH="$BARE" sh "$SH" --answers np.env --print-env 2>/dev/null | sed -n 's/^DATACENTER_ID=//p')
...
BS=$(PATH="$BARE" sh "$SH" --answers np.env --print-env 2>/dev/null | sed -n 's/^POSTGRES_PASSWORD=//p')
...
sh "$SH" --answers k1.env --print-env >/dev/null 2>&1
chk "a bare module NAME is rejected"       fail "$([ $? -ne 0 ] && echo fail || echo ok)"
printf 'DEPLOYMENT=single\nKEY_BACKEND=hsm\n' > k2.env
sh "$SH" --answers k2.env --print-env >/dev/null 2>&1
chk "hsm with no module is rejected"       fail "$([ $? -ne 0 ] && echo fail || echo ok)"
printf 'DEPLOYMENT=single\nKEY_BACKEND=file\n' > k3.env
sh "$SH" --answers k3.env --print-env >/dev/null 2>&1
...
sh "$SH" --answers haloop.env --print-env >/dev/null 2>&1
