...
int main(int argc, char** argv) {
    ...
    if (cmd == "verify") {
        ...
        std::string art = read_file(artifact), sig = read_file(sigfile);
        if (art.empty() || sig.empty()) { std::cerr << "cannot read the artifact or the signature\n"; return 2; }
        bool ok;
        if (pubkey.empty()) {
            // ⚠️ NO KEY CONFIGURED MEANS USE OURS, NOT VERIFY NOTHING. This used to refuse and
            // tell the operator to go and find a key, so the default state of every install
            // was "cannot check a download" — which is the opposite of what pinning is for.
            // The keys we sign with are compiled in; RELEASE_PUBKEY overrides them for anyone
            // signing their own builds.
            ok = pki::verify_release_signature_pinned(art, sig);
        } else {
            std::string pub = read_file(pubkey);
            if (pub.empty()) { std::cerr << "cannot read " << pubkey << "\n"; return 2; }
            ok = pki::verify_release_signature(pub, art, sig);
        }
