...
Point the demo at a real deployment and it acts as a remote client. First
provision a target descriptor (creates a demo `requester` user, reads the CMP
secret / ports / paths from the node):

```bash
demo/provision-target.sh admin@pki.example.org        # SSH target of the node
...
If you SSH in as an account that **didn't** deploy the stack (so the deploy
dir sits under another user's home, e.g. `/home/someone/FastPKI/deploy`), the
script finds and reads it via **passwordless sudo** — that account just needs
`sudo` on the node. If it lacks sudo, point it straight at the dir with
`--compose-dir /home/<owner>/FastPKI/deploy`.
