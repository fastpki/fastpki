...
void handle_pkcsreq(ServerState& st, const httplib::Request& req, httplib::Response& res,
    ...
    try {
        ...
        if (require_challenge) {
            ...
            if (!ok) {
                try {
                    ...
                    ev.actor_ip = client_ip(req); ev.status = pki::audit_status::kFailure;
        ...
        if (!scep_user.empty()) {
            ...
            if (!pki::may_enrol(*st.db, scep_user, role, "scep:enrol", st.instance_id,
                ...
                try {
                    pki::AuditEvent ev;
                    ev.category = pki::audit_cat::kAuth; ev.action = "auth_fail";
                    ev.actor = scep_user; ev.actor_ip = client_ip(req);
        ...
        auto cert = issue_from_csr(st, csr.get(), client_ip(req), txid, serial,
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (do_mint) {
            ...
            // Every check an enrolment will make is made here, at the mistake, by the same
            // function the console issues challenges through (pki/enrol_codes.hpp).
            try {
                const std::string token = pki::issue_scep_challenge(*st.db, st.cfg, mint_profile, mint_ttl);
                std::cout << token << "\n";
                std::cerr << "issued SCEP challenge (profile='" << mint_profile
                          << "', ttl=" << mint_ttl << "s)\n";
            } catch (const std::exception& e) {
                std::cerr << "fastpki-scep: refusing to issue a challenge "
                          << (mint_profile.empty() ? std::string("with no profile")
                                                   : "for profile '" + mint_profile + "'")
                          << ": " << e.what() << "\n";
                return 1;
            }
            return 0;
