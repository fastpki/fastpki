...
echo "=== A console on HTTPS signs the Apple profiles with its own certificate ==="
# The shipped shape: the console's TLS key lives in the token, so the signature is made
# through the PKCS#11 provider. Over plain HTTP (the console above) there is no certificate
# to sign with, and the profile stays plain XML, as the checks above read it.
PORT2=$((PORT + 1))
sed "s/^WEB_PORT=.*/WEB_PORT=$PORT2/" web.conf > web-tls.conf
printf 'WEB_TLS_KEY=%s\nWEB_CERT_ID=web\n' "$(hsm_new_key_uri web-tls)" >> web-tls.conf
hsm_conf_lines >> web-tls.conf
"$WEB" --config web-tls.conf >web-tls.log 2>&1 & P2=$!
trap 'pg_cleanup; kill $P $P2 2>/dev/null' EXIT
wait_conf "web-tls.conf" WEB_PORT "$P2" || true
chk "PRECONDITION: the HTTPS console is up" yes "$(kill -0 $P2 2>/dev/null && echo yes || echo no)"
U2="https://127.0.0.1:$PORT2"
curl -sk -c req2.cj -X POST "$U2/api/login" -d 'username=req1&password=req1pw12345' >/dev/null
chk "the ACME profile downloads over HTTPS (200)" 200 \
    "$(curl -sk -b req2.cj -o signed.mobileconfig -w '%{http_code}' "$U2/api/client-config/appleacme?ca=ca1")"
chk "  it is a CMS signature whose content is the profile" yes \
    "$("$OSSL" cms -verify -inform DER -in signed.mobileconfig -noverify -out inner.xml \
         -signer signer.pem >/dev/null 2>&1 && grep -q '<key>ClientIdentifier</key>' inner.xml && echo yes || echo no)"
echo | "$OSSL" s_client -connect "127.0.0.1:$PORT2" 2>/dev/null | "$OSSL" x509 > served.pem 2>/dev/null
chk "  signed with the certificate the console serves" \
    "$("$OSSL" x509 -in served.pem -noout -fingerprint -sha256 2>/dev/null)" \
    "$("$OSSL" x509 -in signer.pem -noout -fingerprint -sha256 2>/dev/null)"
PLAIN="$(curl -s -b req.cj "$U/api/client-config/appleacme?ca=ca1")"
chk "over plain HTTP the profile stays unsigned XML" yes "$(has "$PLAIN" '<?xml')"
