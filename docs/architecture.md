...
| tables | why |
|---|---|
| `nonces`, `accounts`, `orders`, `authorizations`, `challenges` | ACME session state — see below |
...
**[code] The baseline is 1 and `sql/steps/` is empty.** A schema change requires a new
`sql/steps/000N-*.sql`, an edit to `sql/createdb.sql` and a `kSchemaVersion` bump in one
commit. Steps are immutable once shipped, and `deploy/schema-apply.sh` applies them before
new binaries roll.
