...
# ⚠️ THE DESIGN RECORD IS NOT PUT ON THE WEBSITE, AND SAYS SO ITSELF. docs/architecture.md is
# written for whoever is changing the code, with every claim tagged by how it is known — not
# for somebody deciding whether to deploy FastPKI. It carries `fastpki:not-published` near its
# top and docs/publish.sh reads that.
#
# The marker is IN the file rather than in a list in the script, because a list is two places
# that can disagree: rename the file and the list silently stops matching, the design record
# is published with everything else, and nobody finds out until it is on the site. These
# checks are for the other direction — somebody deleting the marker, or the publisher
# forgetting to honour it.
chk "the design record says it is not published" yes \
    "$(head -30 "$ROOT/docs/architecture.md" | grep -q 'fastpki:not-published' && echo yes || echo no)"
chk "  and publish.sh reads that marker rather than a list of names" yes \
    "$(grep -q "NOT_PUBLISHED_MARKER='fastpki:not-published'" "$ROOT/docs/publish.sh" \
       && grep -q 'grep -q "\$NOT_PUBLISHED_MARKER"' "$ROOT/docs/publish.sh" && echo yes || echo no)"
chk "  and no published guide is named in the exclusion by hand" "" \
    "$(grep -nE '^UNPUBLISHED="[a-z]' "$ROOT/docs/publish.sh" | head -1)"
